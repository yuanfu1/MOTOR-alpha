...
CONTAINS
  ...
  SUBROUTINE rttov_readmw_atlas(dir,month,atlas,verbose,err,lat1,lat2,lon1,lon2)
    ...
    IF (ASSOCIATED(atlas%emis)) THEN
      ...
      THROWM(err .NE. errorstatus_success, "TELSEM2 atlas data structure already allocated")
    ENDIF
...
!     atlas%correspondance(:)=-777
    ...
    IF (verbose) INFO('Reading number of data in atlas...')
    ...
    IF( ios /= 0 ) THEN
       ...
       THROWM(err .NE. errorstatus_success, msg)
    ENDIF
    ...
    IF (verbose) INFO('reading classes...')
