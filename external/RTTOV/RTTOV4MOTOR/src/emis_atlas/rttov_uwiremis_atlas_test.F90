! Description:
!> @file
!!   Executable for basic test of the U. Wisconsin IR emissivity atlas.
!
!> @brief
!!   Executable for basic test of the U. Wisconsin IR emissivity atlas.
!!
!! @details
!!   This should be run using the test_uwiremis_atlas.sh script in rttov_test/
!
! Copyright:
...
!
program rttov_uwiremis_atlas_test
...
#include "throw.h"
  ...
  use mod_uwiremis_atlas, only : numpcs

  implicit none

  real(kind=jprb),    parameter :: difftol = 1.E-9_jprb
  real(kind=jprb),    parameter :: difftolpc = 1.E-15_jprb
  ...
  real(kind=jprb),    allocatable :: pc_eval(:,:)
  real(kind=jprb),    allocatable :: pc_evec(:,:)
  real(kind=jprb),    allocatable :: pc_const(:)
  real(kind=jprb)                 :: emis, pcemis
...
! Generate the chanprof array
  ...
  allocate(pc_eval(numpcs,nprof))
  allocate(pc_evec(numpcs,nchan*nprof))
  allocate(pc_const(nchan*nprof))
...
! Open file for output
  open(ioout, file='output_uwiremis_atlas.ascii', form='formatted', status='replace', action='write')
...
!-------------------------
      if (single_instrument) then
        call rttov_setup_emis_atlas(   &
                    ...
                    atlas,             &  ! UWIRemis atlas is the default IR atlas
                    path = atlas_path, &
                    coefs = coefs,     &
...
! Reconstruct emissivities from PCs and compare
      diffflag = .false.
      do k = 1, nprof
        lo = (k-1)*nchan
        ! PCs are only returned for land surfaces and zero snow fraction
        if (profiles(k)%skin%surftype == 0 .and. &
            profiles(k)%skin%snow_fraction <= 0._jprb) then

          do j = 1, nchan
            emis = emissivity(lo+j,init_run)
            pcemis = sum(pc_eval(:,k) * pc_evec(:,lo+j)) + pc_const(lo+j)

            ! If the returned emissivity is less than zero don't compare
            ! Also ignore case where emissivity has been capped at 1.
            if (emis > 0._jprb .and. &
                .not. (emis == 1._jprb .and. pcemis >= 1._jprb)) then
              diffflag = diffflag .or. abs(emis - pcemis) > difftolpc
            endif
          enddo
        endif
      enddo
      if (diffflag) then
        write(ioout,'(a)') 'WARNING: difference between emissivity and PC-reconstructed emissivity'
      endif
...
end program rttov_uwiremis_atlas_test
