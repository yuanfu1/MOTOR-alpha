! Description:
!> @file
!!   Executable for basic test of the CAMEL IR emissivity atlas.
!
!> @brief
!!   Executable for basic test of the CAMEL IR emissivity atlas.
!!
!! @details
!!   This should be run using the test_camel_atlas.sh script in rttov_test/
!
! Copyright:
...
!
program rttov_camel_atlas_test
...
#include "throw.h"
  ...
  use mod_camel_atlas, only : numpcs
...
! Open file for output
  open(ioout, file='output_camel_atlas.ascii', form='formatted', status='replace', action='write')
...
!-------------------------
      if (single_instrument) then
        call rttov_setup_emis_atlas(   &
                    ...
                    atlas,             &
                    atlas_id = 2_jpim, &  ! To select CAMEL atlas
                    path = atlas_path, &
                    coefs = coefs,     &
...
end program rttov_camel_atlas_test
