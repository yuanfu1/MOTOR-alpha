...
!
SUBROUTINE rttov_write_ascii_sccldcoef(err, coef, coef_scatt, file_id, verbose)
!INTF_OFF
#include "throw.h"
...
!- End of header --------------------------------------------------------
  ...
  IF (coef_scatt%optp_wcl_opac%ntypes > 0) THEN

    section = 'WATER_CLOUD_OPAC'
    ...
    CALL write_ascii_optp(err, file_id, coef, coef_scatt%optp_wcl_opac, 'water cloud', routinename)
    THROW(err.NE.0)

  ENDIF ! wcl_opac properties defined

  IF (coef_scatt%optp_wcl_deff%ntypes > 0) THEN

    section = 'WATER_CLOUD_DEFF'
    WRITE (file_id, '(a)', iostat=err) ' ! ------------------------------------------------------'
    THROW(err.NE.0)
    ...
    CALL write_ascii_optp(err, file_id, coef, coef_scatt%optp_wcl_deff, 'water cloud', routinename)
    THROW(err.NE.0)

  ENDIF ! wcl_deff properties defined

  IF (coef_scatt%optp_icl_baum%ntypes > 0) THEN

    section = 'ICE_CLOUD_BAUM'
    WRITE (file_id, '(a)', iostat=err) ' ! ------------------------------------------------------'
    THROW(err.NE.0)
    ...
    CALL write_ascii_optp(err, file_id, coef, coef_scatt%optp_icl_baum, 'ice cloud', routinename)
    THROW(err.NE.0)

  ENDIF ! icl_baum properties defined
