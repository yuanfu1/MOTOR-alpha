...
!
SUBROUTINE rttov_read_ascii_sccldcoef(err, coef, coef_scatt, file_id, channels)
!INTF_OFF
#include "throw.h"
!INTF_ON
...
!- End of header --------------------------------------------------------
  ...
  readfile : DO
    ...
    CASE ('WATER_CLOUD_OPAC')

      CALL read_ascii_optp(err, file_id, section, coef, coef_scatt%optp_wcl_opac, channels)
      THROW(err.NE.0)

    CASE ('WATER_CLOUD_DEFF')

      CALL read_ascii_optp(err, file_id, section, coef, coef_scatt%optp_wcl_deff, channels)
      THROW(err.NE.0)

    CASE ('ICE_CLOUD_BAUM')

      CALL read_ascii_optp(err, file_id, section, coef, coef_scatt%optp_icl_baum, channels)
      THROW(err.NE.0)
