! Description:
!> @file
!!   Read a binary PC-RTTOV coefficient file, optionally extracting a subset
!!   of channels.
!
!> @brief
!!   Read a binary PC-RTTOV coefficient file, optionally extracting a subset
...
!INTF_OFF
  USE rttov_const, ONLY : rttov_magic_string, rttov_magic_number, sensor_id_hi, &
...
! PRINCOMP_PREDICTORS
  ...
  THROWM(err.NE.0, "reading coef_pccomp%fmv_pc_cld")
  ...
  IF (coef_pccomp%fmv_pc_comp_pc >= 5) THEN
    ...
    THROWM(err.NE.0, "reading coef_pccomp%fmv_pc_aer")

    READ (file_id, iostat=err)coef_pccomp%fmv_pc_nlte
    THROWM(err.NE.0, "reading coef_pccomp%fmv_pc_nlte")
  ELSE
  ...
  THROWM(err.NE.0, "reading coef_pccomp%fmv_pc_bands")

  ALLOCATE (coef_pccomp%fmv_pc_sets(coef_pccomp%fmv_pc_bands), STAT = err)
  THROWM(err.NE.0, "allocation of coef_pccomp%fmv_pc_sets")
  ...
  THROWM(err.NE.0, "reading coef_pccomp%fmv_pc_msets")
...
! coef%fmv_chn is the number of channels that the user requests
  ...
  DO m = 1, coef_pccomp%fmv_pc_bands
    ...
    THROWM(err.NE.0, "allocation of coef_pccomp%eigen(m)%eigenvectors")
    ...
    IF (all_channels_rec) THEN
      DO n = 1, coef_pccomp%fmv_pc_mnum
        ...
        THROWM(err.NE.0,"reading coef_pccomp%eigen(m)%eigenvectors")
      ENDDO
    ELSE
...
! PRINCOMP_COEFFICIENTS
  DO m = 1, coef_pccomp%fmv_pc_bands
    DO n = 1, coef_pccomp%fmv_pc_sets(m)
      ...
      ALLOCATE (coef_pccomp%pcreg(m,n)%coefficients(coef_pccomp%pcreg(m,n)%fmv_pc_npred, &
                                                    coef_pccomp%fmv_pc_mnum), STAT = err)
      THROWM(err.NE.0, "allocation of coef_pccomp%pcreg(m,n)%coefficients")

      DO j = 1, coef_pccomp%fmv_pc_mnum
        READ (file_id, iostat=err)     &
            (coef_pccomp%pcreg(m,n)%coefficients(i, j), i = 1, coef_pccomp%pcreg(m,n)%fmv_pc_npred)
        THROWM(err.NE.0,"reading coef_pccomp%pcreg(m,n)%coefficients")
      ENDDO
    ENDDO
  ENDDO
...
!     PC_PROFILE_LIMITS
  IF (coef_pccomp%fmv_pc_comp_pc >= 5) THEN
    ...
    THROWM(err.NE.0,"reading coef_pccomp%fmv_pc_gas_lim")
  ...
  IF (coef_pccomp%fmv_pc_comp_pc >= 5) THEN
    READ (file_id, iostat=err)coef_pccomp%lim_pc_prfl_gasmin
    THROWM(err.NE.0,"reading coef_pccomp%lim_pc_prfl_gasmin")

    READ (file_id, iostat=err)coef_pccomp%lim_pc_prfl_gasmax
    THROWM(err.NE.0,"reading coef_pccomp%lim_pc_prfl_gasmax")
  ENDIF
  ...
  IF (coef_pccomp%fmv_pc_aer /= 0) THEN
    ...
    THROWM(err.NE.0, "reading coef_pccomp%fmv_pc_naer_types")
    ...
    READ (file_id, iostat=err)coef_pccomp%lim_pc_prfl_aermin
    THROWM(err.NE.0, "reading coef_pccomp%lim_pc_prfl_aermin")

    READ (file_id, iostat=err)coef_pccomp%lim_pc_prfl_aermax
    THROWM(err.NE.0, "reading coef_pccomp%lim_pc_prfl_aermax")
  ENDIF
...
!     INSTRUMENT_NOISE
  ...
  IF (all_channels_rec) THEN
    DO n = 1, coef_pccomp%fmv_pc_nchn
      READ (file_id, iostat=err)coef_pccomp%ff_ori_chn_in(n), coef_pccomp%ff_cwn_in(n), coef_pccomp%ff_bco_in(n),      &
