! Description:
!> @file
!!   Runs the RTTOV tangent linear model with multiple threads under OpenMP.
!
!> @brief
!!   Runs the RTTOV tangent linear model with multiple threads under OpenMP.
!!
...
!!   compiler flags.
!!
!!   The arguments are identical to those for rttov_tl.F90 though
!!   the traj and traj_tl arguments cannot be used with the parallel interface.
!!
...
#include "rttov_errorreport.interface"
  ...
  TYPE(rttov_opt_param),    ALLOCATABLE :: aer_opt_param_tl_x(:)

  TYPE(rttov_opt_param),    ALLOCATABLE :: cld_opt_param_x(:)               ! user cloud opt params
  TYPE(rttov_opt_param),    ALLOCATABLE :: cld_opt_param_tl_x(:)
  ...
  TYPE(rttov_emissivity),   POINTER :: emissivity_tl_x(:)
  ...
  TYPE(rttov_reflectance),  POINTER :: reflectance_tl_x(:)
  ...
  TYPE(rttov_transmission), ALLOCATABLE :: transmission_tl_x(:)
  TYPE(rttov_radiance),     ALLOCATABLE :: radiance_tl_x(:)
  TYPE(rttov_pccomp),       ALLOCATABLE :: pccomp_tl_x(:)
...
!
  ...
  ENDIF
  IF (PRESENT(traj_tl) .AND. opts%config%verbose) THEN
    CALL rttov_errorreport(errorstatus_success, "traj_tl argument will not be used !!", NameOfRoutine)
  ENDIF
...
! Check we do not have more bands than channels
  ...
  IF (mstat .NE. 0) GOTO 100
  ALLOCATE(aer_opt_param_tl_x(nbands), stat = mstat)
  IF (mstat .NE. 0) GOTO 100

  ALLOCATE(cld_opt_param_x(nbands), stat = mstat)
  IF (mstat .NE. 0) GOTO 100
  ALLOCATE(cld_opt_param_tl_x(nbands), stat = mstat)
  IF (mstat .NE. 0) GOTO 100
  ...
  IF (mstat .NE. 0) GOTO 100
  IF (do_pc) THEN
    ALLOCATE(pccomp_tl_x(nbands), stat = mstat)
    IF (mstat .NE. 0) GOTO 100
  ENDIF
  ...
  IF (PRESENT(emissivity_tl)) THEN
    emissivity_tl_x => emissivity_tl
  ELSE
    ALLOCATE(emissivity_tl_x(nchannels), stat = mstat)
    IF (mstat .NE. 0) GOTO 100
  ENDIF
  ...
  IF (PRESENT(reflectance_tl)) THEN
    reflectance_tl_x => reflectance_tl
  ELSE
    ALLOCATE(reflectance_tl_x(nchannels), stat = mstat)
    IF (mstat .NE. 0) GOTO 100
  ENDIF
  ...
  DO iband = 1, nbands
    ...
    ENDIF
    IF (PRESENT(aer_opt_param_tl) .AND. opts%rt_ir%user_aer_opt_param) THEN
      CALL AssociateOptParam(ichannel1(iband), ichannel2(iband), &
        aer_opt_param_tl_x(iband), aer_opt_param_tl)
    ELSE
      NULLIFY(aer_opt_param_tl_x(iband)%abs,     &
              aer_opt_param_tl_x(iband)%sca,     &
              aer_opt_param_tl_x(iband)%bpr,     &
              aer_opt_param_tl_x(iband)%legcoef, &
              aer_opt_param_tl_x(iband)%pha,     &
              aer_opt_param_tl_x(iband)%phangle)
    ENDIF
    ...
    ENDIF
    IF (PRESENT(cld_opt_param_tl) .AND. opts%rt_ir%user_cld_opt_param) THEN
      CALL AssociateOptParam(ichannel1(iband), ichannel2(iband), &
        cld_opt_param_tl_x(iband), cld_opt_param_tl)
    ELSE
      NULLIFY(cld_opt_param_tl_x(iband)%abs,     &
              cld_opt_param_tl_x(iband)%sca,     &
              cld_opt_param_tl_x(iband)%bpr,     &
              cld_opt_param_tl_x(iband)%legcoef, &
              cld_opt_param_tl_x(iband)%pha,     &
              cld_opt_param_tl_x(iband)%phangle)
    ENDIF
    ...
    IF (do_pc) THEN
      IF (do_radrec) THEN
        CALL AssociatePCcomp(ipc1(iband), ipc2(iband), &
          pccomp_tl_x(iband), pccomp_tl, ichannelrec1(iband), ichannelrec2(iband))
      ELSE
        CALL AssociatePCcomp(ipc1(iband), ipc2(iband), &
          pccomp_tl_x(iband), pccomp_tl)
      ENDIF
    ENDIF
...
!    " ichannel1 = ", ichannel1(iband), " ichannel2 = ", ichannel2(iband)
    ...
    ENDIF
    IF (PRESENT(reflectance_tl)) THEN
      WRITE (*, '(A20," = ",A20," (",I5,":",I5,") ")') "refl_tl%refl_in", &
       "refl_tl%refl_in", ichannel1(iband), ichannel2(iband)
      WRITE (*, '(A20," = ",A20," (",I5,":",I5,") ")') "refl_tl%refl_out", &
       "refl_tl%refl_out", ichannel1(iband), ichannel2(iband)
    ENDIF
#endif
    WRITE (*, *)
  ENDIF
  ...
  IF (do_pc) THEN
    CALL &
        ...
        , emissivity_tl    = emissivity_tl_x(ichannel1(iband):ichannel2(iband))  &
        , calcrefl         = calcrefl_x(ichannel1(iband):ichannel2(iband))       &
        , reflectance      = reflectance_x(ichannel1(iband):ichannel2(iband))    &
        , reflectance_tl   = reflectance_tl_x(ichannel1(iband):ichannel2(iband)) &
        , aer_opt_param    = aer_opt_param_x(iband)                              &
        , aer_opt_param_tl = aer_opt_param_tl_x(iband)                           &
        , cld_opt_param    = cld_opt_param_x(iband)                              &
        , cld_opt_param_tl = cld_opt_param_tl_x(iband)                           &
        , pccomp           = pccomp_x(iband)                                     &
        , pccomp_tl        = pccomp_tl_x(iband)                                  &
...
!  ENDDO
  ...
  IF (PRESENT(emissivity_tl)) THEN
    NULLIFY(emissivity_tl_x)
  ELSE
    DEALLOCATE(emissivity_tl_x)
  ENDIF
  ...
  IF (PRESENT(reflectance_tl)) THEN
    NULLIFY(reflectance_tl_x)
  ELSE
    DEALLOCATE(reflectance_tl_x)
  ENDIF

  DEALLOCATE(aer_opt_param_x)
  DEALLOCATE(aer_opt_param_tl_x)

  DEALLOCATE(cld_opt_param_x)
  DEALLOCATE(cld_opt_param_tl_x)
  ...
  IF (do_pc) DEALLOCATE(pccomp_tl_x)
