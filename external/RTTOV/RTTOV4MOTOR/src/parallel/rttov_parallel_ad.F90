! Description:
!> @file
!!   Runs the RTTOV adjoint model with multiple threads under OpenMP.
!
!> @brief
!!   Runs the RTTOV adjoint model with multiple threads under OpenMP.
!!
...
!!   compiler flags.
!!
!!   The arguments are identical to those for rttov_ad.F90 though
!!   the traj and traj_ad arguments cannot be used with the parallel interface.
!!
...
#include "rttov_errorreport.interface"
  ...
  TYPE(rttov_opt_param),    ALLOCATABLE :: aer_opt_param_ad_x(:)

  TYPE(rttov_opt_param),    ALLOCATABLE :: cld_opt_param_x(:)               ! user cloud opt params
  TYPE(rttov_opt_param),    ALLOCATABLE :: cld_opt_param_ad_x(:)
  ...
  TYPE(rttov_emissivity),   POINTER :: emissivity_ad_x(:)
  ...
  TYPE(rttov_reflectance),  POINTER :: reflectance_ad_x(:)
  ...
  TYPE(rttov_transmission), ALLOCATABLE :: transmission_ad_x(:)
...
!
  ...
  ENDIF
  IF (PRESENT(traj_ad) .AND. opts%config%verbose) THEN
    CALL rttov_errorreport(errorstatus_success, "traj_ad argument will not be used !!", NameOfRoutine)
  ENDIF
...
! Check we do not have more bands than channels
  ...
  IF (mstat .NE. 0) GOTO 100
  ALLOCATE(aer_opt_param_ad_x(nbands), stat = mstat)
  IF (mstat .NE. 0) GOTO 100

  ALLOCATE(cld_opt_param_x(nbands), stat = mstat)
  IF (mstat .NE. 0) GOTO 100
  ALLOCATE(cld_opt_param_ad_x(nbands), stat = mstat)
  IF (mstat .NE. 0) GOTO 100
  ...
  IF (PRESENT(emissivity_ad)) THEN
    emissivity_ad_x => emissivity_ad
  ELSE
    ALLOCATE(emissivity_ad_x(nchannels), stat = mstat)
    IF (mstat .NE. 0) GOTO 100
  ENDIF
  ...
  IF (PRESENT(reflectance_ad)) THEN
    reflectance_ad_x => reflectance_ad
  ELSE
    ALLOCATE(reflectance_ad_x(nchannels), stat = mstat)
    IF (mstat .NE. 0) GOTO 100
  ENDIF
  ...
  DO iband = 1, nbands
    ...
    ENDIF
    IF (PRESENT(aer_opt_param_ad) .AND. opts%rt_ir%user_aer_opt_param) THEN
      CALL AssociateOptParam(ichannel1(iband), ichannel2(iband), &
        aer_opt_param_ad_x(iband), aer_opt_param_ad)
    ELSE
      NULLIFY(aer_opt_param_ad_x(iband)%abs,     &
              aer_opt_param_ad_x(iband)%sca,     &
              aer_opt_param_ad_x(iband)%bpr,     &
              aer_opt_param_ad_x(iband)%legcoef, &
              aer_opt_param_ad_x(iband)%pha,     &
              aer_opt_param_ad_x(iband)%phangle)
    ENDIF
    ...
    ENDIF
    IF (PRESENT(cld_opt_param_ad) .AND. opts%rt_ir%user_cld_opt_param) THEN
      CALL AssociateOptParam(ichannel1(iband), ichannel2(iband), &
        cld_opt_param_ad_x(iband), cld_opt_param_ad)
    ELSE
      NULLIFY(cld_opt_param_ad_x(iband)%abs,     &
              cld_opt_param_ad_x(iband)%sca,     &
              cld_opt_param_ad_x(iband)%bpr,     &
              cld_opt_param_ad_x(iband)%legcoef, &
              cld_opt_param_ad_x(iband)%pha,     &
              cld_opt_param_ad_x(iband)%phangle)
    ENDIF
...
!    " ichannel1 = ", ichannel1(iband), " ichannel2 = ", ichannel2(iband)
    ...
    ENDIF
    IF (PRESENT(reflectance_ad)) THEN
      WRITE (*, '(A20," = ",A20," (",I5,":",I5,") ")') "refl_ad%refl_in", &
       "refl_ad%refl_in", ichannel1(iband), ichannel2(iband)
      WRITE (*, '(A20," = ",A20," (",I5,":",I5,") ")') "refl_ad%refl_out", &
       "refl_ad%refl_out", ichannel1(iband), ichannel2(iband)
    ENDIF
#endif
    WRITE (*, *)
  ENDIF
  ...
  IF (do_pc) THEN
    CALL &
        ...
        , emissivity_ad    = emissivity_ad_x(ichannel1(iband):ichannel2(iband))  &
        , calcrefl         = calcrefl_x(ichannel1(iband):ichannel2(iband))       &
        , reflectance      = reflectance_x(ichannel1(iband):ichannel2(iband))    &
        , reflectance_ad   = reflectance_ad_x(ichannel1(iband):ichannel2(iband)) &
        , aer_opt_param    = aer_opt_param_x(iband)                              &
        , aer_opt_param_ad = aer_opt_param_ad_x(iband)                           &
        , cld_opt_param    = cld_opt_param_x(iband)                              &
        , cld_opt_param_ad = cld_opt_param_ad_x(iband)                           &
...
!  ENDDO
  ...
  IF (PRESENT(emissivity_ad)) THEN
    NULLIFY(emissivity_ad_x)
  ELSE
    DEALLOCATE(emissivity_ad_x)
  ENDIF
  ...
  IF (PRESENT(reflectance_ad)) THEN
    NULLIFY(reflectance_ad_x)
  ELSE
    DEALLOCATE(reflectance_ad_x)
  ENDIF

  DEALLOCATE(aer_opt_param_x)
  DEALLOCATE(aer_opt_param_ad_x)

  DEALLOCATE(cld_opt_param_x)
  DEALLOCATE(cld_opt_param_ad_x)
  ...
  IF (do_pc) DEALLOCATE(pccomp_ad_x)
