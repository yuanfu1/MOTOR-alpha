...
!INTF_OFF
use mod_scattering, only: density_wat, density_air, density_ice, density_lmt, density_sphere, cd_snow, cd_grpl, &
...
do i_lvl = 0, m_lvl                                             
  ...
  do i_dia = 1, n_dia
    ...
    if (i_type == i_totalice .or. i_type == i_snow) then
      density_froz = 1e3_jprb * 0.012 / dia_froz_cm (i_dia) ! AJGDB densities now SI, 1e3*old formula g/cm3
      cd_froz      = cd_snow
    else        
      density_froz = density_sphere (i_type)
      cd_froz      = cd_grpl
    end if
...
!* average over melting layer and AJGDB convert to m^2 from cm^2
ext  = ext  / z_km * 1e-4_jprb
sct  = sct  / z_km * 1e-4_jprb
asm  = asm  / z_km * 1e-4_jprb
bsct = bsct / z_km * 1e-4_jprb
