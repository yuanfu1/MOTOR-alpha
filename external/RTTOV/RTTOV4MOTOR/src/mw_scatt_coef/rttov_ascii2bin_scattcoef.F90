! Description:
!> @file
!!   Executable for converting an ASCII RTTOV-SCATT hydrotable file
!!   to binary format.
!
!> @brief
!!   Executable for converting an ASCII RTTOV-SCATT hydrotable file
...
#include "rttov_errorreport.interface"
  ...
  Write(file_id, iostat=io_status) coef_scatt%rttov_version
  ...
  Write(file_id, iostat=io_status) &
         ...
         & coef_scatt%mtemp,       &
         & coef_scatt%mwc,         &
         & associated(coef_scatt%zef) ! Whether reflectivity is present or not

  Write(file_id, iostat=io_status) coef_scatt%freq (:)

  Write(file_id, iostat=io_status) coef_scatt%mpol (:)
  ...
  Write(file_id, iostat=io_status) coef_scatt%is_frozen (:)
  ...
  if (associated(coef_scatt%zef)) write(file_id, iostat=io_status) coef_scatt % zef(:,:,:,:)
