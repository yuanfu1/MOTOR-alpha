function permittivity (i_type, f_ghz, temp, ipermwat, ipermice, density)
...
! Inputs
!   i_type: hydrometeor type as in mod_scattering.F90
!   f_ghz:  frequency [GHz]
!   temp:   temperature [K]
!   ipermwat: (OPTIONAL) water permittivity option as in mod_scattering.F90
!   ipermice: (OPTIONAL) ice permittivity option 
!   density:  (OPTIONAL) if present, and i_type is a frozen hydrometeor, 
!                        return effective permittivity of ice-air mixture [kg m^-3]
...
!INTF_OFF
use mod_scattering,  only: perm_air, density_air, density_ice, permwat_liebe_89, &
  & permwat_rosenkranz_15 , permwat_TKC_16, permice_maetzler_2006, is_frozen
!INTF_ON
...
integer (kind=jpim), optional, intent ( in) :: ipermice
...
if (.not. is_frozen(i_type)) then
  ...
  !* Frozen
  if( .not. present(ipermice) ) then
    stop 'Permittivity model of pure ice must be specified'
  endif

  call perm_ice (f_ghz, temp, ipermice == permice_maetzler_2006, perm_re, perm_im)
