...
! 1.2       20/03/2020  SI throughout (Alan Geer)
...
real (kind=jprb), intent (  out) :: q_ext, q_sct, q_asm, q_bsct   ! [m^2]
...
if (Dinmeters<rtmp) then
  write(*,*) err_base
  write(*,'(A,F12.6,A)') &
    'Smallest maximum diameters given for current habit is ', D_arts(1)*1e2_JPRB, 'cm.'
  write(*,'(A,F12.6,A)') &
    'Extrapolation set to be allowed at maximum down to ', rtmp*1e2_JPRB, 'cm.'
  write(*,'(A,F12.6,A)') &
    'Requested diameter was: ', Dinmeters*1e2_jprb, 'cm.'
  stop
endif
...
if (Dinmeters>rtmp) then
  write(*,*) err_base
  write(*,'(A,F12.6,A)') &
    'Largest maximum diameters given for current habit is ', D_arts(nD)*1e2_JPRB,'cm.'
  write(*,'(A,F12.6,A)') &
    'Extrapolation set to be allowed at maximum up to ', rtmp*1e2_JPRB, 'cm.'
  write(*,'(A,F12.6,A)') &
    'Requested diameter was: ', Dinmeters*1e2_jprb, 'cm.'
  stop
endif
...
q_ext = (ssp_arts(fi,  Ti,  Di,  1)*fw *Tw *Dw  + &
         ...
         ssp_arts(fi+1,Ti+1,Di+1,1)*fw1*Tw1*Dw1)
q_sct = (ssp_arts(fi,  Ti,  Di,  2)*fw *Tw *Dw  + &
         ...
         ssp_arts(fi+1,Ti+1,Di+1,2)*fw1*Tw1*Dw1)
...
q_bsct= (ssp_arts(fi,  Ti,  Di,  4)*fw *Tw *Dw  + &
         ...
         ssp_arts(fi+1,Ti+1,Di+1,4)*fw1*Tw1*Dw1) 
