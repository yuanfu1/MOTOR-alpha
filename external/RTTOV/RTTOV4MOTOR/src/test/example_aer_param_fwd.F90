...
!!
!! @details
!!   This example is most easily run using the run_example_aer_param_fwd.sh
...
!!
!!   The output is written to a file called example_aer_param_fwd_output.dat
!!
...
#include "rttov_skipcommentline.interface"
  ...
  READ(*,*) prof_filename
  WRITE(0,*) 'enter path of aerosol optical parameter file'
...
!     CALL rttov_skipcommentline(iup, errorstatus)
  ...
  DO iprof = 1, nprof
    ...
    DO i = 1, nlevels-1_jpim
      READ(iup,*) aer_opt_param % abs(i,joff:joff+nchannels-1)
    ENDDO
    CALL rttov_skipcommentline(iup, errorstatus)

    DO i = 1, nlevels-1_jpim
      READ(iup,*) aer_opt_param % sca(i,joff:joff+nchannels-1)
    ENDDO
    CALL rttov_skipcommentline(iup, errorstatus)
    ...
    DO i = 1, nlevels-1_jpim
      DO j = 1, nchannels
        READ(iup,*) aer_opt_param % pha(:,i,joff+j-1)
      ENDDO
    ENDDO
    CALL rttov_skipcommentline(iup, errorstatus)
  ENDDO
  ...
  IF (opts % rt_ir % ir_scatt_model == 2) THEN
    ...
    CALL rttov_bpr_init(errorstatus, aer_opt_param % phangle(:))
    IF (errorstatus /= errorstatus_success) THEN
      WRITE(*,*) 'error calling rttov_bpr_init'
      CALL rttov_exit(errorstatus)
    ENDIF

    ! Calculate the b parameter for each layer for each channel
    DO iprof = 1, nprof
      DO j = 1, nchannels
        ...
        DO i = 1, nlevels-1_jpim
          ...
          IF (ANY(aer_opt_param % pha(:,i,j+joff) > 0._jprb)) THEN

            ! If this phase function is same as previous layer copy the bpr
            IF (i > 1 .AND. ALL(aer_opt_param % pha(:,i,j+joff) == aer_opt_param % pha(:,MAX(i-1,1),j+joff))) THEN
              aer_opt_param % bpr(i,j+joff) = aer_opt_param % bpr(i-1,j+joff)
            ELSE
              CALL rttov_bpr_calc(errorstatus, &
                                  aer_opt_param % pha(:,i,j+joff), &! input phase function
                                  aer_opt_param % phangle(:),      &! input phase angles
                                  aer_opt_param % bpr(i,j+joff),   &! output b parameter
                                  nthreads)
              IF (errorstatus /= errorstatus_success) THEN
          ...
          ELSE
            aer_opt_param % bpr(i,j+joff) = 0._jprb
          ENDIF
      ...
      DO j = 1, nchannels
        ...
        DO i = 1, nlevels-1_jpim
          ...
          IF (ANY(aer_opt_param % pha(:,i,j+joff) > 0._jprb)) THEN
            ...
            IF (i > 1 .AND. ALL(aer_opt_param % pha(:,i,j+joff) == aer_opt_param % pha(:,MAX(i-1,1),j+joff))) THEN
              aer_opt_param % legcoef(:,i,j+joff) = aer_opt_param % legcoef(:,i-1,j+joff)
            ELSE
              CALL rttov_legcoef_calc(errorstatus, &
                                  aer_opt_param % pha(:,i,j+joff),   &! input phase function
                                  aer_opt_param % phangle(:),        &! input phase angles
                                  opts % rt_ir % dom_nstreams,       &! number of Legendre coefficients to calculate
                                  aer_opt_param % legcoef(:,i,j+joff))! output Legendre coefficients
              IF (errorstatus /= errorstatus_success) THEN
          ...
          ELSE
            aer_opt_param % legcoef(:,i,j+joff) = 0._jprb
          ENDIF
