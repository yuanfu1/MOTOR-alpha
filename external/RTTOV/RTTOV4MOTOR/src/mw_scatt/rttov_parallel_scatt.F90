! Description:
!> @file
!!   Runs the RTTOV-SCATT direct model with multiple threads under OpenMP.
!
!> @brief
!!   Runs the RTTOV-SCATT direct model with multiple threads under OpenMP.
!!
...
!!   compiler flags.
!!
!!   The arguments are identical to those for rttov_scatt.F90.
!!
...
#include "rttov_errorreport.interface"
  ...
  REAL(KIND=jprb),           POINTER     :: cfrac_x(:)
  TYPE(rttov_scatt_emis_retrieval_type), &
                             ALLOCATABLE :: emis_retrieval_terms_x(:)
...
! Check we do not have more bands than channels
  ...
  IF (PRESENT(cfrac)) THEN
    cfrac_x => cfrac
  ELSE
    ALLOCATE(cfrac_x(nprofiles), stat = mstat)
    IF (mstat .NE. 0) GOTO 100
  ENDIF
  ...
  ENDDO
  !
  ! do the same for emis_retrieval_terms structure
  !
  IF (PRESENT(emis_retrieval_terms)) THEN
    ALLOCATE(emis_retrieval_terms_x(nbands), stat = mstat)
    IF (mstat .NE. 0) GOTO 100
    DO iband = 1, nbands

      CALL AssociateEmisRetrievalTerms(ichannel1(iband), ichannel2(iband), &
         emis_retrieval_terms_x(iband), emis_retrieval_terms)
...
!    " ichannel1 = ", ichannel1(iband), " ichannel2 = ", ichannel2(iband)
    ...
    IF (PRESENT(reflectivity)) THEN
      CALL rttov_scatt                                                         &
        ...
        , radiance_x(iband)                                                    &
        , cfrac_x(iprofile1(iband):iprofile2(iband)) &
        , reflectivity = reflectivity_x(iband))
    ELSEIF (PRESENT(emis_retrieval_terms)) THEN
      CALL rttov_scatt                                                         &
        (errorstatus_x(iband)                                                  &
        , opts_scatt                                                           &
        , nlevels                                                              &
...
!  ENDDO
  ...
  ! Set the plane_parallel flag in the radiance structure
  radiance%plane_parallel = ANY(radiance_x(:)%plane_parallel)
  ...
  ENDIF
  IF (PRESENT(emis_retrieval_terms)) THEN
    DEALLOCATE(emis_retrieval_terms_x)
  ENDIF
...
CONTAINS
  ...
  !> Set up one emis_retrieval_terms structure to point to a subset of another
  !! @param[in]     ichannel1       first channel in range of subset
  !! @param[in]     ichannel2       last channel in range of subset
  !! @param[out]    emis_terms_x    emis_retrieval_terms structure pointing to subset of user structure
  !! @param[in]     emis_terms      user emis_retrieval_terms structure
  SUBROUTINE AssociateEmisRetrievalTerms(ichannel1, ichannel2, emis_terms_x, emis_terms)
    INTEGER(KIND=jpim),                    INTENT(IN)  :: ichannel1, ichannel2
    TYPE(rttov_scatt_emis_retrieval_type), INTENT(IN)  :: emis_terms
    TYPE(rttov_scatt_emis_retrieval_type), INTENT(OUT) :: emis_terms_x

    emis_terms_x % cfrac &
      => emis_terms % cfrac(ichannel1:ichannel2)

    emis_terms_x % bsfc &
      => emis_terms % bsfc(ichannel1:ichannel2)

    emis_terms_x % tau_cld &
      => emis_terms % tau_cld(ichannel1:ichannel2)

    emis_terms_x % up_cld &
      => emis_terms % up_cld(ichannel1:ichannel2)

    emis_terms_x % down_cld &
      => emis_terms % down_cld(ichannel1:ichannel2)

    emis_terms_x % tau_clr &
      => emis_terms % tau_clr(ichannel1:ichannel2)

    emis_terms_x % up_clr &
      => emis_terms % up_clr(ichannel1:ichannel2)

    emis_terms_x % down_clr &
      => emis_terms % down_clr(ichannel1:ichannel2)
