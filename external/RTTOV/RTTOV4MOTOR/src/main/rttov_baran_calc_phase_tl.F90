! Description:
!> @file
!!   TL of phase function for ice cloud from Baran parameterisation
!
!> @brief
!!   TL of phase function for ice cloud from Baran parameterisation
!!
!! @details
!!   This also computes the direct model phase function.
!!
!! @param[in]     g                asymmetry parameter
!! @param[in]     g_tl             asymmetry parameter perturbation
!! @param[in]     phangle          angles on which to calculate phase function (degrees)
!! @param[out]    phase            phase function
!! @param[out]    phase_tl         phase function perturbations
!
! Copyright:
...
!INTF_END
  ...
  REAL(KIND=JPRB) :: AA, AA_TL
  REAL(KIND=JPRB) :: BB, BB_TL
  REAL(KIND=JPRB) :: P,  P_TL
  REAL(KIND=JPRB) :: X
  REAL(KIND=JPRB) :: ALPHA, ALPHA_TL
  REAL(KIND=JPRB) :: NORM,  NORM_TL
  REAL(KIND=JPRB) :: BETA
  REAL(KIND=JPRB) :: DD,   DD_TL
  ...
  IF ( ( G .LT. 0.2_JPRB ) .AND. ( G .GE. 0.0_JPRB ) ) THEN
    DO J=1,NPHANGLE
      ...
      BB_TL =  1.5_JPRB * (1.0_JPRB+G*G-2.0_JPRB*G*COS(X))**0.5_JPRB *&
           & ( 2.0_JPRB*G_TL*G - 2.0_JPRB*G_TL*COS(X))
      P    = (AA/BB)
      P_TL = ( AA_TL * BB - AA * BB_TL )/ BB**2_JPIM
      PHASE(J)    = P
      PHASE_TL(J) = P_TL
    ENDDO
  ENDIF
  ...
  IF ( ( G .LT. 0.7_JPRB ) .AND. ( G .GE. 0.2_JPRB ) ) THEN
    IF ( ( G .LT. 0.7_JPRB ) .AND. ( G .GE. 0.6_JPRB ) ) THEN
      ...
      ALPHA_TL = - (G_TL*ALPHA) / (2.0_JPRB*G)
    ENDIF
    ...
    IF ( ( G .LT. 0.3_JPRB ) .AND. ( G .GE. 0.2_JPRB ) ) THEN
      ...
      ALPHA_TL =  (G_TL*ALPHA) / (2.0_JPRB*(1.0_JPRB-G))
    ENDIF
    DO J=1_JPIM,NPHANGLE
      ...
      IF ( V .LT. 54.8_JPRB ) THEN
        BB=(1.0_JPRB+G*G-2.0_JPRB*G*COS(X))**1.5_JPRB
        BB_TL =  1.5_JPRB * (1.0_JPRB+G*G-2.0_JPRB*G*COS(X))**0.5_JPRB *&
           & ( G - COS(X))* ( 2.0_JPRB*G_TL)
        P=(AA/BB)*COS(X)*ALPHA
        P_TL = (( AA_TL * BB - AA * BB_TL ) / BB**2_JPIM )* COS(X)*ALPHA + &
               & (AA/BB)*COS(X)*ALPHA_TL
      ENDIF
      IF ( V .GE. 54.8_JPRB ) THEN
        BB=(1.0_JPRB+G*G-1.8_JPRB*G*COS(X)*SIN(X))**1.5_JPRB
        BB_TL = 1.5_JPRB * (1.0_JPRB+G*G-1.8_JPRB*G*COS(X)*SIN(X))**0.5_JPRB *&
              & ( 2.0_JPRB*G -1.8_JPRB*COS(X)*SIN(X) )*G_TL
  ...
  IF ( G .GE. 0.7_JPRB ) THEN
    IF ( ( G .LE. 0.8_JPRB ) .AND. ( G .GE. 0.7_JPRB ) ) THEN
      ...
      NORM_TL = -0.2025E+03*G_TL + 2.0_JPRB*0.4949E+02*G_TL*G
    ENDIF
    IF ( ( G .LE. 0.9_JPRB ) .AND. ( G .GT. 0.8_JPRB ) ) THEN
      NORM=0.2771E+03-0.5102E+03*G+0.2329E+03*G*G
      NORM_TL = -0.5102E+03*G_TL + 2.0_JPRB*0.2329E+03*G_TL*G
    ENDIF
    IF ( G .GT. 0.90_JPRB ) THEN
      NORM=0.4219E+03-0.8271E+03*G+0.4063E+03*G*G
      NORM_TL = -0.8271E+03*G_TL + 2.0_JPRB*0.4063E+03*G_TL*G
    ENDIF
    ALPHA=NORM/SQRT(G)
    ALPHA_TL = (NORM_TL*SQRT(G) - 0.5_JPRB*ALPHA*G_TL) / G
    ...
    DO J = 1_JPIM,NPHANGLE
      ...
      IF ( V .LE. 3.0_JPRB) THEN
        ...
        P_TL = (COS(X))**128.0_JPRB * ((( AA_TL * BB - AA * BB_TL ) / BB**2_JPIM ) * ALPHA + &
               & (AA/BB)*ALPHA_TL)
      ENDIF
      IF ( ( V .GT. 3.0_JPRB ) .AND. ( V .LT. 30.0_JPRB ) ) THEN
        BB=(1.0_JPRB+G*G-2.0_JPRB*G*COS(1.3_JPRB*X))**1.2_JPRB
        BB_TL= 1.2_JPRB * (1.0_JPRB+G*G-2.0_JPRB*G*COS(1.3_JPRB*X))**0.2_JPRB *&
           & ( G - COS(1.3_JPRB*X)) * 2.0_JPRB*G_TL
        P=(AA/BB)*COS(X)
        P_TL = (( AA_TL * BB - AA * BB_TL ) / BB**2_JPIM )* COS(X)
      ENDIF
      IF ( ( V .GE. 30.0_JPRB ) .AND. ( V .LT. 54.8_JPRB ) ) THEN
        DD=((1.0_JPRB-G)/4.6_JPRB)+G
        DD_TL = (1.0_JPRB - 1.0_JPRB/4.6_JPRB)*G_TL
        ...
        BB_TL= BETA * (1.0_JPRB+G*G-2.0_JPRB*G*COS(DD*X))**(BETA-1.0_JPRB) *&
           & ( 2.0_JPRB*G_TL*G - 2.0_JPRB*G_TL*COS(DD*X) + 2.0_JPRB*G*SIN(DD*X)*DD_TL*X)
        P=(AA/BB)*COS(X)
        P_TL = (( AA_TL * BB - AA * BB_TL ) / BB**2_JPIM )* COS(X)
      ENDIF
      IF ( ( V .GE. 54.8_JPRB ) .AND. ( V .LE. 95.0_JPRB ) ) THEN
        BB=(1.0_JPRB+G*G-1.5_JPRB*G*COS(X)*SIN(X))**1.5_JPRB
        BB_TL = 1.5_JPRB * (1.0_JPRB+G*G-1.5_JPRB*G*COS(X)*SIN(X))**0.5_JPRB *&
              & ( 2.0_JPRB*G_TL*G -1.5_JPRB*G_TL*COS(X)*SIN(X))
        P=AA/BB
        P_TL = ( AA_TL * BB - AA * BB_TL )/ BB**2_JPIM
      ENDIF
