...
MODULE rttov_const
  ...
  INTEGER(jpim), PARAMETER :: mfasislut_version_compatible_max = 1
  ...
  ! -------------------------------------------------------
  ! Platform ID codes
  INTEGER(jpim), PARAMETER :: nplatforms = 63
  ...
  INTEGER(jpim), PARAMETER :: &
         ...
         platform_id_forum     = 58, platform_id_tempestd  = 59, platform_id_jasoncs   = 60, &
  ...
  CHARACTER(LEN=len_platform), PARAMETER :: platform_name(nplatforms) = &
         (/ 'noaa     ', 'dmsp     ', 'meteosat ', 'goes     ', 'gms      ', &
            ...
            'cloudsat ', 'cloudcore', 'forum    ', 'tempestd ', 'jasoncs  ', &
            'electro-l', 'oms      ', 'sentinel2' /)
  ...
  INTEGER(jpim), PARAMETER :: &
         ...
         inst_id_saphirng   = 108, inst_id_forum    = 109, inst_id_tempestd = 110, inst_id_marss    = 111, &
         ...
         inst_id_sntnl2_msi = 120

  INTEGER(jpim), PARAMETER :: ninst = 121
  ...
  CHARACTER(LEN=len_instrument), PARAMETER :: inst_name(0:ninst-1) =     &
          (/ 'hirs    ', 'msu     ', 'ssu     ', 'amsua   ', 'amsub   ', &
             ...
             'tempestd', 'marss   ', 'virr    ', 'sirs    ', 'hirasfsr', &
  ...
  ! -------------------------------------------------------
  ! Temperature
  REAL(jprb), PARAMETER :: tmax   = 400.0_jprb       ! degK
  REAL(jprb), PARAMETER :: tmin   = 90.0_jprb        ! degK
  ! Water Vapour
  ...
  REAL(KIND=jprb), PARAMETER :: min_reflectivity = -999.0_jprb
  ! Output a tiny radiance as indicator that radar simulator radiances are not to be trusted
  REAL(KIND=jprb), PARAMETER :: min_radiance_radar = 1.0E-8_jprb
  ! Minimum allowable hydrometeor fraction in radar simulator
  REAL(KIND=jprb), PARAMETER :: min_cfrac_radar = 1.0E-4_jprb
