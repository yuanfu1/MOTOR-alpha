...
!!   coefficient files to v12 format.
!
!> @brief
!!   Executable for converting RTTOV v10/v11-compatible
!!   coefficient files to v12 format.
...
!!   file and \-\-coef-out specifies the output v12-
...
!
PROGRAM rttov11_conv_coef_11to12
...
#include "rttov11_read_ascii_coef.interface"
#include "rttov_write_ascii_coef.interface"
#include "rttov11_read_hdf5_coef.interface"
...
#include "rttov_hdf_save.interface"
  ...
  CALL initoptions( "This program converts RTTOV v10/v11 optical depth coefficient files to v12 format")
#ifndef _RTTOV_HDF
  CALL getoption("--coef-in", f_coef_in, mnd=.TRUE._jplm, use="Input ASCII v10/v11 optical depth coefficient file")
  CALL getoption("--coef-out", f_coef_out, mnd=.TRUE._jplm, use="Output ASCII v12 optical depth coefficient file")
#else
  CALL getoption("--coef-in", f_coef_in, mnd=.TRUE._jplm, use="Input ASCII/HDF5 v10/v11 optical depth coefficient file")
  CALL getoption("--coef-out", f_coef_out, mnd=.TRUE._jplm, use="Output ASCII/HDF5 v12 optical depth coefficient file")
#endif
  CALL checkoptions()
  ...
  ELSEIF (rttov_cmpuc(file_format, 'formatted')) THEN
    CALL rttov11_read_ascii_coef(err, file_id, coef)
  ...
  ELSEIF (rttov_cmpuc(file_format, 'hdf5')) THEN
    ...
    CALL rttov11_read_hdf5_coef(err, TRIM(f_coef_in), coef)
  ...
  IF (TRIM(file_format) == 'formatted') THEN
    ...
    CALL rttov_write_ascii_coef(err, coef, file_id)
...
#ifdef _RTTOV_HDF
    CALL rttov_hdf_save(err, f_coef_out, "/COEF", create=.TRUE._jplm, coef=coef, compress=.TRUE._jplm)
  ...
  PCATCH
END PROGRAM rttov11_conv_coef_11to12
