...
if ((matplotlib_version.split('.')[0] == '1' and
     matplotlib_version.split('.')[1] >= '4') or
        matplotlib_version.split('.')[0] >= '2'):
...
def format_coord(axes, xdata, ydata):
    ...
    if xdata is None or ydata is None:
        return ''
    # P format_coord doesn't work if axes' ticks are defined
#    print "value:",axes.format_coord(xdata, ydata)
#    print xdata, ydata
    xstr = "%0.4g" % xdata
    ystr = "%0.4g" % ydata

#    print type(axes.format_coord(xdata, ydata))
    value = str(axes.get_ylabel()) + "=" + ystr + "  " + \
        str(axes.get_xlabel()) + "=" + xstr
...
class AxesLimits:
    ...
    def __init__(self, autoscaleUnzoom, y_inverse=False):
    ...
    def set(self, axes, xrange, yrange):
        ...
        # for our purpose pression is always inverted so ...
        axes.set_ylim((yrange[1], yrange[0]))
        # print "wxmplot set" ,yrange,(yrange[1],yrange[0])
        return True
...
def update_postscript_resolution(printData):
    ...
    stuck at 72 DPI.
    """
    if not isinstance(getattr(wx, 'PostScriptDC_SetResolution', None), collections.Callable):
        return
...
class PlotPanel(FigureCanvasWxAgg):
    ...
    def __init__(self, parent, id, size=(6.0, 3.70), dpi=96, cursor=True,
        ...
        # print "create self.cursor",cursor
        ...
        # patch http://matplotlib.1069221.n5.nabble.com/wxMPL-patch-td34955.html
        # the following changed according to Robin Dunn's advice for 2.9
        # -- but it probably wasn't working right before!
        #topwin.Connect(wx.ID_ANY, self.GetId(), wx.wxEVT_ACTIVATE,self.OnActivate)
    ...
    def draw(self, **kwds):
        ...
        """
        # print "wxmplot class PlotPanel draw ",self.GetClassName(),self.GetName(),self.GetId(),self.GetLabel()
...
class StripCharter:
    ...
    def __init__(self, axes, loc_legend="upper right"):
        """
    ...
    def _create_plot(self):
        ...
        if self.channels:
            ...
            self.axes.legend(lines, labels, numpoints=2, loc=self.loc_legend,
    ...
    def _update_channel(self, channel, zoomed):
        ...
        if zoomed:
            ...
            # test pb when redraw suprresion last point if zoomzd
            return True
        else:
            return True
