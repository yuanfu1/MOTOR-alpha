        ...
        if ( len_trim(aaction) == 4 ) then
            ...
            if ( aaction == 'read' ) then
                if ( present( stat ) ) then
                    ...
                    call error_stop( msg = 'In ' // module_name // ' % ' //         &
...
999     if (present(stat) ) then
            ...
            call self % log_io_error( 'Unable to open ' // trim(filename), &
                                      ...
                                      iostat = iostat,                     &
                                      iomsg = iomsg )
            call error_stop( msg = module_name // ' % ' // procedure_name // &
                ': Unable to open file')
        end if
...
! Check that unit is not input_unit
            if ( unit == input_unit ) then
                if ( present(stat) ) then
                    ...
                    call error_stop( msg =  'unit in ' // module_name // ' % ' // &
...
!! stopping.
        ...
        character(10) :: action
         ...
         call error_stop( msg =  'write failure in ' // module_name // '.')
...
999     if ( present(stat) ) then
            ...
            call error_stop( msg =  'close_unit failed in ' // module_name // ' % ' // &
                procedure_name // '.')
        end if
