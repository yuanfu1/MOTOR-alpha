...
! For int8, radix sort becomes counting sort, so buffer is not needed
    ...
    module subroutine int8_radix_sort(array, reverse)
    ...
    module subroutine int16_radix_sort(array, work, reverse)
        ...
        if (present(work)) then
            if (size(work, kind=int_index) < N) then
                call error_stop( msg = "int16_radix_sort: work array is too small.")
            end if
...
! Use binary search to find the boundary, and move them to correct position.
    ...
    module subroutine int32_radix_sort(array, work, reverse)
        ...
        if (present(work)) then
            if (size(work, kind=int_index) < N) then
                call error_stop( msg = "int32_radix_sort: work array is too small.")
            end if
            use_internal_buffer = .false.
            buffer => work
        else
    ...
    module subroutine sp_radix_sort(array, work, reverse)
        ...
        if (present(work)) then
            if (size(work, kind=int_index) < N) then
                call error_stop( msg = "sp_radix_sort: work array is too small.")
            end if
            use_internal_buffer = .false.
...
! thus if the array contains `NaN`, the result is undefined.
    ...
    module subroutine int64_radix_sort(array, work, reverse)
        ...
        if (present(work)) then
            if (size(work, kind=int_index) < N) then
                call error_stop( msg = "int64_radix_sort: work array is too small.")
            end if
            use_internal_buffer = .false.
            buffer => work
        else
