...
contains
   ...
   function dictionary_get_scalar(self,key,required,error) result(scalar)
      ...
      if (required.and..not.associated(node)) then
         ...
         print *, '!!->> Warning! Error in get_scalar (Yaml): ', trim(error%message), " value not change."
      end if
      if (associated(node)) then
         select type (node)
            ...
            class is (type_null)
               ...
               print *, '!!->> Warning! Error in get_scalar (Yaml): ', trim(error%message)
            class is (type_dictionary)
               allocate(error)
   ...
   function dictionary_get_dictionary(self,key,required,error) result(dictionary)
      ...
      if (required.and..not.associated(node)) then
         ! allocate(error)
         error%message = trim(self%path)//' does not contain key "'//trim(key)//'".'
         print *, '!!->> Warning! Error in get_dictionary (Yaml): ', trim(error%message)
      end if
      if (associated(node)) then
   ...
   function dictionary_get_list(self,key,required,error) result(list)
      ...
      if (required.and..not.associated(node)) then
         allocate(error)
         error%message = trim(self%path)//' does not contain key "'//trim(key)//'".'
         print *, '!!->> Warning! Error in get_list (Yaml): ', trim(error%message)
      end if
      if (associated(node)) then
         select type (typed_node=>node)
            class is (type_null)
   ...
   function dictionary_get_string(self,key,default,error) result(value)
      ...
      ELSE
         allocate(error)
         error%message = trim(key)//' is not presented.'
         print *, '!!->> Warning! Error in get_string (Yaml): ', trim(error%message)
      END IF
   end function
   ...
   function dictionary_get_logical(self,key,default,error) result(value)
      ...
      if (associated(node)) then
         ...
         if (.not.success) then
            ...
            print *, '!!->> Warning! Error in get_logical (Yaml): ', trim(error%message)
         end if
      end if
   end function
   ...
   function dictionary_get_integer(self,key,default,error) result(value)
      ...
      if (present(default)) value = default
      ! node => self%get_scalar(key,.not.present(default),error)
      node => self%get_scalar(key, .TRUE. ,error)
      if (associated(node)) then
         ...
         if (.not.success) then
            ...
            print *, '!!->> Warning! Error in get_integer (Yaml): ', trim(error%message)
         end if
      end if
   end function
   ...
   function dictionary_get_real(self,key,default,error) result(value)
      ...
      if (associated(node)) then
         ...
         if (.not.success) then
            ...
            print *, '!!->> Warning! Error in get_real (Yaml): ', trim(error%message)
         end if
      end if
   end function
