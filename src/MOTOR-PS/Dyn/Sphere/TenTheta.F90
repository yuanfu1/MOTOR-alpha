...
CONTAINS
  ...
  SUBROUTINE Tendency(this, theta, div, psi, chi, w, ten_theta, X)
    IMPLICIT NONE
    ...
    REAL(r_kind), DIMENSION(:, :), ALLOCATABLE :: psi2, chi2, div2, omg, J_theta_psi, &
    ...
    ASSOCIATE (num_cell => this%sg%num_cell, &
      ...
      ALLOCATE (psi2(vLevel, num_cell), chi2(vLevel, num_cell), omg(vLevel, num_cell), &
                ...
                F_z_chi(vLevel, num_cell), div2(vLevel, num_cell))
      ...
      J_theta_psi = 0.0D0
      F_theta_chi = 0.0D0
      partheta_parsigma = 0.0D0
      J_z_psi = 0.0D0
      F_z_chi = 0.0D0

      ! PRINT *, 'size of omg and w are: ', SIZE(omg, dim=2), size(w, dim=2), num_iCell

      omg(:, 1:num_icell) = this%sg%Hz(:, 1:num_iCell) * w(:, 1:num_iCell)
      ...
      ! IF (PRESENT(X)) THEN
      !    X%fields(X%getVarIdx('J_z_psiF'))%data(:, :, 1) = J_z_psi
      !    X%fields(X%getVarIdx('F_z_chiF'))%data(:, :, 1) = F_z_chi
      !    X%fields(X%getVarIdx('J_th_psi'))%data(:, :, 1) = J_theta_psi
      !    X%fields(X%getVarIdx('F_th_chi'))%data(:, :, 1) = F_theta_chi
      !    X%fields(X%getVarIdx('psiF'))%data(:, :, 1) = psi2
      !    X%fields(X%getVarIdx('chiF'))%data(:, :, 1) = chi2
      ! END IF
  ...
  SUBROUTINE destroy(this)
    IMPLICIT NONE
    CLASS(TenTheta_t), INTENT(INOUT) :: this
