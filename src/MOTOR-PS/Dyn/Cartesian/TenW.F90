  ...
  SUBROUTINE Tendency(this, w, div, psi, chi, rho, P, ten_w)
    ...
    ASSOCIATE (num_cell => this%sg%num_cell, &
      ...
      ALLOCATE (psi2(vLevel, num_cell), chi2(vLevel, num_cell), &
                div2(vLevel, num_cell), J_w_psi(vLevel, num_cell), &
                F_w_chi(vLevel, num_cell), parw_parsigma(vLevel, num_cell), &
                F_z_chi(vLevel, num_cell), parP_parsigma(vLevel, num_cell), &
                J_z_psi(vLevel, num_cell), omg(vLevel, num_cell))
      ...
      CALL this%CalVerDer%FirstOrder(P, parP_parsigma)
      ...
      ten_w(:, 1:num_iCell) = J_w_psi(:, 1:num_iCell) - F_w_chi(:, 1:num_iCell) &
                              ...
                              * (J_z_psi(:, 1:num_icell) - F_z_chi(:, 1:num_iCell)) &
                              ...
                              - this%sg%Hz(:, 1:num_iCell) * this%PreCal%invRho(:, 1:num_iCell) &
                              * parP_parsigma(:, 1:num_iCell)

    END ASSOCIATE

    DEALLOCATE (psi2, chi2, div2, J_w_psi, &
                F_w_chi, parw_parsigma, &
                J_z_psi, F_z_chi, &
                parP_parsigma, omg)
