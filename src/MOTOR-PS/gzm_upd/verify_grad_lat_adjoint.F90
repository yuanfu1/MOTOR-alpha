PROGRAM verify_grad_lat_adjoint
  ...
  REAL(r_kind), POINTER :: oprand(:, :), adj_oprand(:, :), adj_grad_lat(:, :), grad_lat(:, :), grad_lon(:, :)
  ...
  file_name = "adjoint_grad_lat_ad_results.txt"
  error_file_name = "grad_lat_large_errors.txt"
  ...
  ALLOCATE (oprand(vLevel, num_cell), adj_oprand(vLevel, num_cell), adj_grad_lat(vLevel, num_cell))
  ...
  adj_grad_lat = 1.0_R_KIND  ! Set adjoint gradient for latitude to 1.0

  ! Forward calculation
  CALL gzm%Gradient(oprand, grad_lat, grad_lon)

  ! Adjoint calculation
  CALL gzm_adj%Grad_Lat_AD(oprand, adj_oprand, adj_grad_lat)
  ...
  DO i = 1, num_cell
    DO k = 1, vLevel
      ...
      CALL compute_numeric_gradient(gzm, oprand, grad_lat, grad_lon, epsilon, k, i, numeric_grad, "Grad_Lat")

      ! Verify adjoint for each grid point
      CALL verify_adjoint(adj_oprand(k, i), numeric_grad, "Grad_Lat", k, i, max_diff, sum_error, sum_square_error, &
  ...
  ! Print final metrics
  WRITE (unit_num, *) "Final Metrics for Grad_Lat:"
  CALL print_final_metrics("Grad_Lat", max_diff, mean_error, std_dev_error, mean_relative_error, std_dev_relative_error, global_pass, tolerance, unit_num)

  CLOSE (unit_num)
  CLOSE (error_unit_num)

  ! Deallocate arrays
  DEALLOCATE (oprand, adj_oprand, adj_grad_lat)
...
CONTAINS
  ...
  SUBROUTINE compute_numeric_gradient(gzm, oprand, grad_lat, grad_lon, epsilon, k, i, numeric_grad, order)
    ...
    IF (TRIM(order) == "Grad_Lat") THEN
      oprand(k, i) = oprand(k, i) + epsilon
      CALL gzm%Gradient(oprand, grad_lat, grad_lon)
      value_plus = grad_lat(k, i)

      oprand(k, i) = oprand(k, i) - 2.0_R_KIND * epsilon
      CALL gzm%Gradient(oprand, grad_lat, grad_lon)
      value_minus = grad_lat(k, i)
...
END PROGRAM verify_grad_lat_adjoint
