PROGRAM verify_laplacia_adjoint
  ...
  file_name = "adjoint_laplacia_ad_results.txt"
  error_file_name = "laplacia_large_errors.txt"
  ...
  DO i = 1, num_cell
    DO k = 1, vLevel
      ...
      CALL compute_numeric_gradient(gzm, opr_left, opr_right, oprand, VALUE, epsilon, k, i, numeric_grad, "Laplacia")

      ! Verify adjoint for each grid point
      CALL verify_adjoint(adj_oprand(k, i), numeric_grad, "Laplacia", k, i, max_diff, sum_error, sum_square_error, &
  ...
  ! Print final metrics
  WRITE (unit_num, *) "Final Metrics for Laplacia:"
  CALL print_final_metrics("Laplacia", max_diff, mean_error, std_dev_error, mean_relative_error, std_dev_relative_error, global_pass, tolerance, unit_num)
...
END PROGRAM verify_laplacia_adjoint
