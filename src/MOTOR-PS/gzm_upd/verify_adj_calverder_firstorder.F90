PROGRAM verify_adj_calverder_firstorder
  ...
  REAL(r_kind), POINTER :: A(:, :), parA_sigma(:, :), A_adj(:, :), parA_sigma_adj(:, :), rhs_adj(:, :)
  ...
  ALLOCATE (rhs_adj(vLevel, num_cell))  ! Allocate rhs_adj for adjoint computation
  ...
  ! Adjoint calculation (outside the loop) using updated FirstOrder_AD
  CALL calverder_adj%FirstOrder_AD(A_adj, parA_sigma_adj, rhs_adj)
  ...
  ! Deallocate arrays
  DEALLOCATE (A, parA_sigma, A_adj, parA_sigma_adj, rhs_adj)
