PROGRAM verify_adj_calverder_firstorder_half
  ...
  REAL(r_kind), POINTER :: A(:, :), parA_sigma_half(:, :), A_adj(:, :), parA_sigma_half_adj(:, :), rhs_adj(:, :)
  ...
  file_name = "adjoint_calverder_firstorder_half_results.txt"
  unit_num = 10
  ...
  ALLOCATE (A(vLevel, num_cell), parA_sigma_half(vLevel, num_cell))
  ALLOCATE (A_adj(vLevel, num_cell), parA_sigma_half_adj(vLevel, num_cell))
  ALLOCATE (rhs_adj(vLevel, num_cell))  ! Allocate rhs_adj for adjoint calculation
  ...
  rhs_adj = 0.0_R_KIND  ! Initialize rhs_adj for adjoint calculation
  ...
  ! Adjoint calculation (outside the loop) using updated FirstOrder_half_AD
  CALL calverder_adj%FirstOrder_half_AD(A_adj, parA_sigma_half_adj, rhs_adj)
  ...
  DO i = 1, num_cell
    DO k = 1, vLevel
      ...
      CALL verify_adjoint(A_adj(k, i), numeric_grad, "FirstOrderHalf", k, i, max_diff, sum_error, sum_square_error, &
  ...
  CALL print_final_metrics("FirstOrderHalf", max_diff, mean_error, std_dev_error, mean_relative_error, std_dev_relative_error, global_pass, tolerance, unit_num)

  CLOSE (unit_num)

  ! Deallocate arrays
  DEALLOCATE (A, parA_sigma_half, A_adj, parA_sigma_half_adj, rhs_adj)
...
CONTAINS
  ...
  SUBROUTINE compute_numeric_gradient(calverder, A, parA_sigma_half, epsilon, k, i, numeric_grad, order)
    ...
    IF (TRIM(order) == "FirstOrderHalf") THEN
      A(k, i) = A(k, i) + epsilon
      CALL calverder%FirstOrder_half(A, parA_sigma_half)
      parA_sigma_plus = SUM(parA_sigma_half)

      A(k, i) = A(k, i) - 2.0_R_KIND * epsilon
      CALL calverder%FirstOrder_half(A, parA_sigma_half)
      parA_sigma_minus = SUM(parA_sigma_half)
    END IF
...
END PROGRAM verify_adj_calverder_firstorder_half
