PROGRAM verify_divergen_adjoint
  ...
  file_name = "adjoint_divergen_ad_results.txt"
  error_file_name = "divergen_large_errors.txt"
  ...
  DO i = 1, num_cell
    DO k = 1, vLevel
      ...
      CALL compute_numeric_gradient(gzm, opr_left, opr_right, oprand, VALUE, epsilon, k, i, numeric_grad, "Divergen")

      ! Verify adjoint for each grid point
      CALL verify_adjoint(adj_opr_left(k, i), numeric_grad, "Divergen", k, i, max_diff, sum_error, sum_square_error, &
  ...
  ! Print final metrics
  WRITE (unit_num, *) "Final Metrics for Divergen:"
  CALL print_final_metrics("Divergen", max_diff, mean_error, std_dev_error, mean_relative_error, std_dev_relative_error, global_pass, tolerance, unit_num)
...
CONTAINS
  ...
  SUBROUTINE compute_numeric_gradient(gzm, opr_left, opr_right, oprand, VALUE, epsilon, k, i, numeric_grad, order)
    ...
    IF (TRIM(order) == "Divergen") THEN
      opr_left(k, i) = opr_left(k, i) + epsilon
      CALL gzm%Divergen(opr_left, opr_right, VALUE)
      value_plus = SUM(VALUE)

      opr_left(k, i) = opr_left(k, i) - 2.0_R_KIND * epsilon
      CALL gzm%Divergen(opr_left, opr_right, VALUE)
...
END PROGRAM verify_divergen_adjoint
