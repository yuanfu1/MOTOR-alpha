PROGRAM verify_adj_calverder_secondorder
  ...
  REAL(r_kind), POINTER :: A(:, :), parA_sigma_2nd(:, :), A_adj(:, :), parA_sigma_2nd_adj(:, :), rhs_adj(:, :)
  ...
  tolerance_rate = 0.9999_R_KIND  ! 99.95% pass rate

  file_name = "adjoint_calverder_secondorder_results.txt"
  unit_num = 10
  ...
  ALLOCATE (A(vLevel, num_cell), parA_sigma_2nd(vLevel, num_cell))
  ALLOCATE (A_adj(vLevel, num_cell), parA_sigma_2nd_adj(vLevel, num_cell))
  ALLOCATE (rhs_adj(vLevel, num_cell))
  ...
  rhs_adj = 0.0_R_KIND  ! Initialize rhs_adj for adjoint computation
  ...
  CALL calverder_adj%SecondOrder_AD(A_adj, parA_sigma_2nd_adj, rhs_adj)
  ...
  DO i = 1, num_cell
    DO k = 1, vLevel
      ...
      CALL verify_adjoint(A_adj(k, i), numeric_grad, "SecondOrder", k, i, max_diff, sum_error, sum_square_error, &
  ...
  CALL print_final_metrics("SecondOrder", max_diff, mean_error, std_dev_error, mean_relative_error, std_dev_relative_error, global_pass, tolerance, unit_num)

  CLOSE (unit_num)

  ! Deallocate arrays
  DEALLOCATE (A, parA_sigma_2nd, A_adj, parA_sigma_2nd_adj, rhs_adj)
...
CONTAINS
  ...
  SUBROUTINE compute_numeric_gradient(calverder, A, parA_sigma_2nd, epsilon, k, i, numeric_grad, order)
    ...
    IF (TRIM(order) == "SecondOrder") THEN
      A(k, i) = A(k, i) + epsilon
      CALL calverder%SecondOrder(A, parA_sigma_2nd)
      parA_sigma_plus = SUM(parA_sigma_2nd)

      A(k, i) = A(k, i) - 2.0_R_KIND * epsilon
      CALL calverder%SecondOrder(A, parA_sigma_2nd)
      parA_sigma_minus = SUM(parA_sigma_2nd)
    END IF
...
END PROGRAM verify_adj_calverder_secondorder
