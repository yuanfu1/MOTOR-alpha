  ...
  SUBROUTINE Laplacia(this, oprand, VALUE)
    ...
    ASSOCIATE (sg => this%sg)
      ...
      DO ic = 1, sg%num_icell  ! Apply to inner points only, icell has halo points etc.
        ...
        DO ie = 1, sg%num_edge
          ...
          DO ig = 1, sg%numQuadPerEdge
            ...
            DO i = 1, UBOUND(sg%edge_stcl, 1)
              ...
              ! IF (abs(ic - 2177) .LE. 3) THEN
              !    PRINT *, 'ig and i are', ig, i
              !    PRINT *, 'coef_norm is ', sg%coef_norm(i, ig, ie, ic)
              ! END IF
            END DO
        ...
        ! IF (abs(ic - 2177) .LE. 3) THEN
        !    PRINT *, 'ic is', ic
        !    PRINT *, 'sg%numQuadPerEdge', sg%numQuadPerEdge
        !    PRINT *, 'sg%coef_gl', sg%coef_gl
        !    PRINT *, 'sg%edge_leng', sg%edge_leng(:, ic)
        !    PRINT *, 'sg%cell_area', sg%cell_area(ic)
        ! END IF
      END DO
