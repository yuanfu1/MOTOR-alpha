...
MODULE RawMWHS_m
  ...
  TYPE :: RawMWHS_t
    ...
    INTEGER(i_kind) :: npixel, nscan, cal_nobs, mwhs_nobs
    CHARACTER(len=50) :: inst_name = 'mwhs2', platform_name = 'fy3_4' !FY3D
    INTEGER(i_kind), ALLOCATABLE, DIMENSION(:) :: chan_lists, ifuse

    ! Defined for ObsMWHS(ObsBase) type
  ...
  CONTAINS
    ...
    PROCEDURE, PUBLIC  :: RawMWHS_setup
    PROCEDURE, PUBLIC  :: RawMWHS_read
    PROCEDURE, PUBLIC  :: Destroy_RawMWHS
    ! PROCEDURE, PRIVATE :: Write_RawObs_fy4_RawMWHS

  END TYPE RawMWHS_t

CONTAINS

  SUBROUTINE RawMWHS_setup(this, configFile, idxFile)
    IMPLICIT NONE
    CLASS(RawMWHS_t) :: this
    ...
    PRINT *, "RawMWHS_setup successfully run"

  END SUBROUTINE RawMWHS_setup

  SUBROUTINE RawMWHS_read(this, radFile)
    USE NETCDF
    USE HDF5
    USE AdvanceTime_m
    IMPLICIT NONE
    ...
    this%nscan = dims(2)
    this%npixel = dims(1)
    this%mwhs_nobs = this%npixel * this%nscan
    this%nRawObs = this%mwhs_nobs
    DEALLOCATE (dims)

    ! Defined in this LOCAL SUBROUTINE
    ALLOCATE (obstime_2D(this%npixel, this%nscan))
    ! Defined and saved for ObsMWHS(ObsBase) type
    ...
    ! Defined for raw data dimensions and for Write_RawObs_fy5_mwhs
    ...
    ALLOCATE (ObsAngle_2D(2, this%nscan))
    ...
    ! PRINT *, "mwhs_read successfully run"

    ! PRINT *, 'DEBUG only: Write_RawObs_fy5_mwhs'
    ! IF(yaml_get_var(this%configFile, 'IO', 'output_dir', outputDir) /= 0) STOP
    ! CALL Write_RawObs_fy5_mwhs(this, TRIM(outputDir)//'/Raw_FY3_MWHS.nc')
    ...
    PRINT *, 'RawMWHS_read is successfully run'

  END SUBROUTINE RawMWHS_read

  SUBROUTINE Write_RawObs_fy5_mwhs(this, filename)
  ...
  END SUBROUTINE Write_RawObs_fy5_mwhs

  SUBROUTINE Destroy_RawMWHS(this)
    CLASS(RawMWHS_t) :: this
  ...
  END SUBROUTINE Destroy_RawMWHS

END MODULE RawMWHS_m
