...
PROGRAM Process_mwts
  ...
  USE parameters_m
  USE Obs2State_m
  USE ReadWriteMWTS_m, ONLY: ReadWriteMWTS_t
  ...
  TYPE(ReadWriteMWTS_t) :: ReadWriteMWTS
  ...
  IF (TRIM(configFile) .EQ. '') THEN
    ...
    PRINT *, TRIM(configFile)
    WRITE (*, 1)
1   FORMAT('Usage of this driver: mpirun -n <n> Debug/Process_mwts.exe configFile', /, &
...
2   FORMAT('ConfigFile is: ', A)
  ...
  DO i_inst = 1, n_insts
    ...
    CALL ReadWriteMWTS%ObsInitial(configFile, i_inst)
    CALL ReadWriteMWTS%ObsIngest(XbMG(mgEnd))

    PRINT *, 'Num of OBS = ', ReadWriteMWTS%numObs
    ...
    ASSOCIATE (sg => geometry%mg%sg(mgEnd))
      ...
      IF (XbRef%sg%isActiveProc()) THEN
        ...
        CALL ReadWriteMWTS%ObsPrepareForSg(XbRef)
        CALL ReadWriteMWTS%OutputForThinning(XbRef, mpObs)
        CALL ReadWriteMWTS%destructor()
        PRINT *, 'ReadWriteMWTS%OutputForThinning OVER'
        PRINT *, 'here ', ALLOCATED(ReadWriteMWTS%rttov_chan_lists), ALLOCATED(ReadWriteMWTS%ObsData)
