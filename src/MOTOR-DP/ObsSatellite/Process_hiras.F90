!!--------------------------------------------------------------------------------------------------
! PROJECT           : MOTOR-DP.Process_hiras
...
PROGRAM Process_hiras
  ...
  USE parameters_m
  USE Obs2State_m
  USE ReadWriteHIRAS_m,  ONLY: ReadWriteHIRAS_t
  ...
  TYPE(ReadWriteHIRAS_t) :: ReadWriteHIRAS
  ...
  IF (TRIM(configFile) .EQ. '') THEN
    ...
    PRINT*, TRIM(configFile)
    WRITE (*, 1)
1   FORMAT('Usage of this driver: mpirun -n <n> Debug/Process_hiras.exe configFile', /, &
...
2   FORMAT('ConfigFile is: ', A)
  ...
  CALL ReadWriteHIRAS%ObsInitial(configFile)
  CALL ReadWriteHIRAS%ObsIngest(XbMG(mgEnd))

  PRINT *, 'Num of OBS = ', ReadWriteHIRAS%numObs
  ...
  ASSOCIATE (sg => geometry%mg%sg(mgEnd))
    ...
    IF (XbRef%sg%isActiveProc()) THEN
      ...
      CALL ReadWriteHIRAS%ObsPrepareForSg(XbRef)  
      CALL ReadWriteHIRAS%OutputForThinning(XbRef, mpObs)
      CALL ReadWriteHIRAS%destructor()
      PRINT *, 'ReadWriteHIRAS%OutputForThinning OVER'
      PRINT *, 'here ', ALLOCATED(ReadWriteHIRAS%rttov_chan_lists), ALLOCATED(ReadWriteHIRAS%ObsData)
