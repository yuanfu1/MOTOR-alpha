...
!   Modified by Yuanfu Xie (yuanfu_xie@yahoo.com), 2022/09/17, @GBA-MWF, Shenzhen
!     For modifying yaml parameters to pass its unit test; Modified for obs files missing
!!--------------------------------------------------------------------------------------------------
!!
!> @brief
...
PROGRAM unitTest_Convention
  ...
  INTEGER(i_kind) :: i, iii, imx, iv, ii
  REAL(r_kind) :: amx
  ...
  configFile = TRIM(configFile)//"/UnitTest/testConvention_MultiObs.yaml"
  ...
  DO i = mgStart, mgEnd
    ...
    BLOCK
      ...
      ASSOCIATE (sg => geometry%mg%sg(i))
        ...
        IF (ObsFlag_Surface) CALL OBS1%ObsThinning(XbMG(i), Y1, mpObs, .TRUE., .FALSE.)
        IF (ObsFlag_Sound) CALL OBS2%ObsThinning(XbMG(i), Y2, mpObs, .TRUE., .FALSE.)
        IF (ObsFlag_Vwpw) CALL OBS3%ObsThinning(XbMG(i), Y3, mpObs, .TRUE., .FALSE.)
