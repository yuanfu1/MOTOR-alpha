...
PROGRAM testIOGrapesModelvar
  ...
  TYPE(IOGrapesModelvar_t), TARGET :: ioGrapesModelvar
  TYPE(State_t)  :: Xm
  ...
  configFile = TRIM(configFile)//"/UnitTest/testIOGrapesModelvar.yaml"
  ...
  ASSOCIATE (sg => geometry%mg%sg(geometry%mg%mg_finest))
    ...
    CALL ioGrapesModelvar%m_read_bcg_into_Xm_Ens(Xm, sg)

    ! Output Xm to NC files
    DO iv = 1, varNum
      varName = varList(iv)
      CALL Output_NC_State_SV(Xm, TRIM(outputDir), "testIOGrapesModelvar_MultiDA_bak_"//TRIM(varName), TRIM(varName))
    END DO
...
END PROGRAM testIOGrapesModelvar
