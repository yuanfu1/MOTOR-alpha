...
! VERSION           : V 0.0   Calculating BEC in multigrid and save as temporary files.
! HISTORY           :
...
PROGRAM Test_EnLoc_Multigrid
  ...
  configFile = TRIM(configFile)//"/Application/App_CMA_GD_EnLoc.yaml"
  ...
  IF (mpddGlob%isBaseProc()) THEN
    DO i = mgStart, mgEnd
      ...
      ! CALL EnLoc%b_qrSave(i)
      ! PRINT *, 'Done EnLoc%b_qrSave in grid', i

      ! CALL EnLoc%b_qrLoad(i)
      ! PRINT *, 'Done EnLoc%b_qrLoad in grid', i

      CALL EnLoc%b_qrRinv()
      PRINT *, 'Done EnLoc%b_qrRinv in grid', i

      CALL EnLoc%b_FWD(i)
      PRINT *, 'Done EnLoc%b_FWD in grid', i
...
END PROGRAM Test_EnLoc_Multigrid
