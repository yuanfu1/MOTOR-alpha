...
PROGRAM Test_ObsRadarVel
  ...
  configFile = "/Users/qzl/sources/MOTOR/input/220527_0000_3km/App_BMatTLAD.yaml"
  ...
  ASSOCIATE (sg => geometry%mg%sg(5))
    BLOCK
      ...
      END BLOCK
      CALL B%initialize(configFile, sg, 'Laplace')
      ...
      BLOCK
        Y = B.SQRTINVMUL.X; 
        rs1 = Y.DOT. (B.SQRTINVMUL.X)
        rs2 = (B%sqrt_inverse_multiply_adjoint(Y, X)) .DOT.X
        IF (mpddGlob%isBaseProc()) THEN
          PRINT *, 'Y*(B^-1/2*X):', rs1
          PRINT *, 'B^-1/2^T*X*Y:', rs2
        END IF
      END BLOCK
