PROGRAM test_gzm
  ...
  ! configFile = "/home/xinyu/project/MOTOR/static/gzm_and_laplace_terrain.yaml"
  ...
  ASSOCIATE (sg => geometry%mg%sg(5), &
    ...
    ! DO i =1, sg%num_cell
    !    IF (sg%bdy_type(i) .NE. 0) vor1(:,i)=vort1(:,i)
    ! END DO
    etat1 = vor1 + sg%f
  ...
  ASSOCIATE (sg => geometry%mg%sg(6), &
    ...
    ! DO i =1, sg%num_cell
    !    IF (sg%bdy_type(i) .NE. 0) vor2(:,i)=vort2(:,i)
    ! END DO
    etat2 = vor2 + sg%f
  ...
  ASSOCIATE (sg => geometry%mg%sg(7), &
    ...
    DO k = 1, vLevel
      ...
      CALL Rossby%RHV_func(0.0D0, sg%cell_cntr, streamtt3, vorticitytt3, &
                           thicks3, derivS3, derivV3, jacobitt3, flxdivtt3)
    ...
    ! DO i =1, sg%num_cell
    !    IF (sg%bdy_type(i) .NE. 0) vor3(:,i)=vort3(:,i)
    ! END DO
    etat3 = vor3 + sg%f
  ...
  PRINT *, 'error_v ratio is ', errorv_1, errorv_2, errorv_3, errorv_1 / errorv_2, errorv_2 / errorv_3
  PRINT *, 'error_f ratio is ', errorf_1, errorf_2, errorf_3, errorf_1 / errorf_2, errorf_2 / errorf_3
  PRINT *, 'error_j ratio is ', errorj_1, errorj_2, errorj_3, errorj_1 / errorj_2, errorj_2 / errorj_3
