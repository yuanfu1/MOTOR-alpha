...
PROGRAM NL_features_RTTOV
  ...
  BLOCK
    ...
    BLOCK
      ...
      ! REAL :: max_v = 2.2, min_v = -0.8, interval_v = 0.2
      ...
      INTEGER :: level_temp = 20, level_qv = 27, level_qc = 22, level_qi = 22
      ! 30-10km; 41-16km; 40-15.8km

      nvalues = int((max_v - min_v)/interval_v) + 1
      nlevels = SIZE(X%fields(X%getVarIdx("qice"))%data,1)
      ...
      ! DO ivalue = 1,49
      !   Xnew = X
      !   level_qi = nlevels - ivalue + 1
      !   Xnew%fields(Xnew%getVarIdx("qice"))%data(level_qi, :, :) = (1+1.5) * X%fields(X%getVarIdx("qice"))%data(level_qi, :, :)
      !   NL = H%fwdNL_opr(Xnew)
      !   NL1 = H%fwdNL_opr(X)
      !   PRINT *, ivalue, level_qi, NL1%ObsFields(2)%values(1), NL%ObsFields(2)%values(1)
      ! END DO
      ...
      DO ivalue = 1, nvalues  
        ...
        Xnew%fields(Xnew%getVarIdx("qice"))%data(level_qi, :, :) = (1+ratio(ivalue)) * X%fields(X%getVarIdx("qice"))%data(level_qi, :, :)
        NL = H%fwdNL_opr(Xnew)
        ! print *, 'NLvalues: ', level_qi, ratio(ivalue), Xnew%fields(Xnew%getVarIdx("qice"))%data(level_qi, 1, 1), NL%ObsFields(2)%values(1)
        ...
        tout(ivalue) = Xnew%fields(Xnew%getVarIdx("qice"))%data(level_qi, 1, 1)
      END DO
      OPEN (12, file = "./NL_features_QI.txt", form="formatted")
      ! WRITE(12, *) "tout at 30th level, qvout at 30th level, RTTOV forward value"
      DO ivalue = 1, nvalues
        WRITE(12, *) tout(ivalue), NLvalue(ivalue),TLvalue(ivalue)
      END DO
      CLOSE(12)
      OPEN (12, file = "./NL_features_QI.bin", form="unformatted", access='stream')
      WRITE(12) tout
      WRITE(12) NLvalue
      WRITE(12) TLvalue
      CLOSE(12)
