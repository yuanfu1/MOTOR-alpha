...
PROGRAM NL_features_RTTOV
  ...
  BLOCK
    ...
    BLOCK
      ...
      ! REAL :: max_v = 5, min_v = -1, interval_v = 0.4
      ! REAL :: max_v2 = 5, min_v2 = -1, interval_v2 = 0.4
      ...
      INTEGER :: level_temp = 22, level_qv = 25, level_qc = 22, level_qi = 22
      ...
      ! PRINT *, 'temp = ', X%fields(X%getVarIdx("temp"))%data(:, 1,1)
      PRINT *, 'qvapor = ', X%fields(X%getVarIdx("qvapor"))%data(:, 1,1)
      ! dX = X
      ! DO ivalue_qc = 1, nvalues1
      !   DO ivalue_qi = 1, nvalues2
      !     Xnew = X
      !     level_temp = 22
      !     level_temp = nlevels - level_temp + 1
      !     Xnew%fields(Xnew%getVarIdx("temp"))%data(level_temp, :, :) = (1+ratio1(ivalue_qc, ivalue_qi)) * X%fields(X%getVarIdx("temp"))%data(level_temp, :, :)
      !     level_qv = 25
      !     level_qv = nlevels - level_qv + 1
      !     Xnew%fields(Xnew%getVarIdx("qvapor"))%data(level_qv, :, :) = (1+ratio2(ivalue_qc, ivalue_qi)) * X%fields(X%getVarIdx("qvapor"))%data(level_qv, :, :)
      !     ! print*, Xnew%fields(Xnew%getVarIdx("temp"))%data(level_temp, 1, 1), Xnew%fields(Xnew%getVarIdx("qvapor"))%data(level_qv, 1, 1)
      !     NL = H%fwdNL_opr(Xnew)
      ...
      !     tout(ivalue_qc, ivalue_qi) = Xnew%fields(Xnew%getVarIdx("temp"))%data(level_temp, 1, 1)
      !     qvout(ivalue_qc, ivalue_qi) = Xnew%fields(Xnew%getVarIdx("qvapor"))%data(level_qv, 1, 1)
      !   END DO
      ! END DO

      ! OPEN (12, file = "./NL_features_TQ.txt", form="formatted")
      ! ! WRITE(12, *) "tout at 30th level, qvout at 30th level, RTTOV forward value"
      ! DO ivalue_qc = 1, nvalues1
      !   DO ivalue_qi = 1, nvalues2
      !     WRITE(12, *) tout(ivalue_qc, ivalue_qi), qvout(ivalue_qc, ivalue_qi), NLvalue(ivalue_qc, ivalue_qi), TLvalue(ivalue_qc, ivalue_qi)
      !   END DO
      ! END DO
      ! CLOSE(12)
      ! OPEN (12, file = "./NL_features_TQ.bin", form="unformatted", access='stream')
      ! WRITE(12) tout
      ! WRITE(12) qvout
      ! WRITE(12) NLvalue
      ! WRITE(12) TLvalue
