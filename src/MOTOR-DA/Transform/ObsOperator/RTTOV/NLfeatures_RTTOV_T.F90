...
PROGRAM NL_features_RTTOV
  ...
  BLOCK
    ...
    BLOCK
      ...
      REAL :: max_v = 0.01, min_v = -0.01, interval_v = 0.001
      ...
      DO ivalue = 1, nvalues 
        ...
        Xnew%fields(Xnew%getVarIdx("temp"))%data(level_temp, :, :) = (1+ratio(ivalue)) * X%fields(X%getVarIdx("temp"))%data(level_temp, :, :)
        NL = H%fwdNL_opr(Xnew)
        ! print *, 'NLvalues: ', level_temp, ratio(ivalue), Xnew%fields(Xnew%getVarIdx("temp"))%data(level_temp, 1, 1), NL%ObsFields(2)%values(1)
        ...
        tout(ivalue) = Xnew%fields(Xnew%getVarIdx("temp"))%data(level_temp, 1, 1)
      END DO
      OPEN (12, file = "./NL_features_T.txt", form="formatted")
      ! WRITE(12, *) "tout at 30th level, qvout at 30th level, RTTOV forward value"
      DO ivalue = 1, nvalues
        WRITE(12, *) tout(ivalue), NLvalue(ivalue),TLvalue(ivalue)
      END DO
      CLOSE(12)
      OPEN (12, file = "./NL_features_T.bin", form="unformatted", access='stream')
      WRITE(12) tout
      WRITE(12) NLvalue
      WRITE(12) TLvalue
      CLOSE(12)
