...
PROGRAM NL_features_RTTOV
  ...
  BLOCK
    ...
    BLOCK
      ...
      INTEGER :: level_temp = 20, level_qv = 20, level_qc = 33, level_qi = 33
      ! 30-10km; 41-16km; 40-15.8km

      nvalues = int((max_v - min_v)/interval_v) + 1
      nlevels = SIZE(X%fields(X%getVarIdx("qvapor"))%data,1)
      ...
      DO ivalue = 1, nvalues  
        ...
        Xnew%fields(Xnew%getVarIdx("qvapor"))%data(level_qv, :, :) = (1+ratio(ivalue)) * X%fields(X%getVarIdx("qvapor"))%data(level_qv, :, :)
        NL = H%fwdNL_opr(Xnew)
        ! print *, 'NLvalues: ', level_qv, ratio(ivalue), Xnew%fields(Xnew%getVarIdx("qvapor"))%data(level_qv, 1, 1), NL%ObsFields(2)%values(1)
        ...
        tout(ivalue) = Xnew%fields(Xnew%getVarIdx("qvapor"))%data(level_qv, 1, 1)
      END DO
      OPEN (12, file = "./NL_features_Q.txt", form="formatted")
      ! WRITE(12, *) "tout at 30th level, qvout at 30th level, RTTOV forward value"
      DO ivalue = 1, nvalues
        WRITE(12, *) tout(ivalue), NLvalue(ivalue),TLvalue(ivalue)
      END DO
      CLOSE(12)
      OPEN (12, file = "./NL_features_Q.bin", form="unformatted", access='stream')
      WRITE(12) tout
      WRITE(12) NLvalue
      WRITE(12) TLvalue
      CLOSE(12)
