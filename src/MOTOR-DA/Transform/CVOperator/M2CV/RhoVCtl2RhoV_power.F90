...
!> @brief
!! This module contains the RhoVCtl2RhoV_Power Object which is tranform the variables from control variable space to
!! model space.
...
MODULE RhoVCtl2RhoV_Power_m
  ...
  TYPE, EXTENDS(M2MBase_t) :: RhoVCtl2RhoV_Power_t
  ...
  CONTAINS
    ...
    FINAL :: destructor
  END TYPE RhoVCtl2RhoV_Power_t

  INTERFACE RhoVCtl2RhoV_Power_t
    PROCEDURE :: constructor
  END INTERFACE RhoVCtl2RhoV_Power_t

CONTAINS

  FUNCTION constructor(configFile, X) RESULT(this)
    IMPLICIT NONE
    TYPE(RhoVCtl2RhoV_Power_t) :: this
  ...
  IMPURE ELEMENTAL SUBROUTINE destructor(this)
    IMPLICIT NONE
    TYPE(RhoVCtl2RhoV_Power_t), INTENT(INOUT) :: this

  END SUBROUTINE destructor

  SUBROUTINE transBackward(this, X)
    IMPLICIT NONE
    CLASS(RhoVCtl2RhoV_Power_t) :: this
    ...
    DO j = LBOUND(X%fields, 1), UBOUND(X%fields, 1)
      ...
      IF ('rhov' .EQ. (TRIM(X%fields(j)%Get_Name()))) THEN
        ...
        PRINT *, 'check rhov_ctl ', MAXVAL(X%fields(j)%DATA), MINVAL(X%fields(j)%DATA)
  ...
  SUBROUTINE transAdjMultiply(this, dX, X)
    ...
    DO j = LBOUND(dX%fields, 1), UBOUND(dX%fields, 1)
      ...
      IF ('rhov' .EQ. (TRIM(dX%fields(j)%Get_Name()))) THEN
        ...
        dX%fields(j)%DATA = dX%fields(j)%DATA * EXP(LOG(p * X%fields(X%getVarIdx('rhov_ctl'))%DATA + 1.0D0) / p) &
                            / (1.0D0 + p * X%fields(X%getVarIdx('rhov_ctl'))%DATA)
        PRINT *, 'check AD calc grad(x_ctl): ', MAXVAL(dX%fields(j)%DATA), MINVAL(dX%fields(j)%DATA)
  ...
  SUBROUTINE transFwdTanLinear(this, dX, X)
    ...
    DO j = LBOUND(dX%fields, 1), UBOUND(dX%fields, 1)
      ...
      IF ('rhov_ctl' .EQ. (TRIM(dX%fields(j)%Get_Name()))) THEN
  ...
  FUNCTION transFwdTanLinear_opr(this, dX, X) RESULT(dX1)
    ...
    dX1 = dX%zeroCopy()
    IF (dX%getVarIdx('rhov_ctl') .NE. 0) THEN
      dX1%fields(dX1%getVarIdx('rhov_ctl')) = dX%fields(dX%getVarIdx('rhov_ctl'))
      CALL this%transFwdTanLinear(dX1, X)
    END IF

  END FUNCTION

END MODULE RhoVCtl2RhoV_Power_m
