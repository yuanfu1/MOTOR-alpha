...
!> @brief
!! This module contains the UV2Divg Object which is tranform the variables from control variable space to
!! model space.
MODULE UV2Divg_m
  ...
  TYPE, EXTENDS(M2MBase_t) :: UV2Divg_t
  ...
  END INTERFACE UV2Divg_t

CONTAINS

  SUBROUTINE calParameters(this)
    IMPLICIT NONE
    CLASS(UV2Divg_t) :: this
    INTEGER(i_kind) :: i, j, k
    REAL(r_kind) :: t1, t2, t3
    REAL(r_kind), ALLOCATABLE :: psMatrix(:, :), tempHeight, tempZs
    ...
    ASSOCIATE (sg => this%X%sg)
      ...
      BLOCK
        ...
        DO i = 1, sg%num_icell
          IF ((sg%cell_stcl(6, i) == 0) .OR. (sg%cell_stcl(8, i) == 0)) CYCLE
          pZspx(i) = (sg%zHght(1, sg%cell_stcl(6, i)) + sg%zHght(1, sg%cell_stcl(9, i)) &
                      - sg%zHght(1, sg%cell_stcl(5, i)) - sg%zHght(1, sg%cell_stcl(8, i))) / (sg%cell_dist(6, i)) / 2.0D0
          pZspy(i) = (sg%zHght(1, sg%cell_stcl(8, i)) + sg%zHght(1, sg%cell_stcl(9, i)) &
                      - sg%zHght(1, sg%cell_stcl(5, i)) - sg%zHght(1, sg%cell_stcl(6, i))) / (sg%cell_dist(8, i)) / 2.0D0
          DO j = 1, sg%vLevel - 1
            tempHeight = (sg%zHght(j, sg%cell_stcl(6, i)) + sg%zHght(j, sg%cell_stcl(9, i)) &
                    + sg%zHght(j, sg%cell_stcl(5, i)) + sg%zHght(j, sg%cell_stcl(8, i))) / 8.0D0 + &
                   (sg%zHght(j + 1, sg%cell_stcl(6, i)) + sg%zHght(j + 1, sg%cell_stcl(9, i)) &
                    + sg%zHght(j + 1, sg%cell_stcl(5, i)) + sg%zHght(j + 1, sg%cell_stcl(8, i))) / 8.0D0
            tempZs = (sg%zHght(1, sg%cell_stcl(6, i)) + sg%zHght(1, sg%cell_stcl(9, i)) &
                    + sg%zHght(1, sg%cell_stcl(5, i)) + sg%zHght(1, sg%cell_stcl(8, i))) / 4.0D0
            this%c1(j, i) = sg%zTop * (sg%ztop - tempHeight) / ((sg%ztop - tempZs)**2) * pZspx(i)
            this%c2(j, i) = sg%zTop * (sg%ztop - tempHeight) / ((sg%ztop - tempZs)**2) * pZspy(i)
            this%c3(i) = sg%zTop / (sg%zTop - tempZs)
          END DO
        END DO
      ...
      BLOCK
        ...
        REAL(r_kind), ALLOCATABLE :: coef_p_pX(:, :)      ! 4 stcl at horizontal / num_icell
        REAL(r_kind), ALLOCATABLE :: coef_p_pY(:, :)      ! 4 stcl at horizontal / num_icell
        REAL(r_kind), ALLOCATABLE :: coef_p_pSigma(:, :)  ! 3 stcl at verticl/ 4 stcl at horizontal / vLevel / num_icell

        INTEGER(i_kind), ALLOCATABLE :: stcl_p_pX(:, :)       ! 4 stcl / num_icell
        INTEGER(i_kind), ALLOCATABLE :: stcl_p_pY(:, :)       ! 4 stcl / num_icell
        INTEGER(i_kind), ALLOCATABLE :: stcl_p_pSigma(:, :)   ! 3 stcl / vLevel

        ALLOCATE (this%coef_p_pX(4, sg%num_cell), &      ! 3 stcl at verticl/ 4 stcl at horizontal/ num_icell
                  this%coef_p_pY(4, sg%num_cell), &      ! 3 stcl at verticl/ 4 stcl at horizontal/ num_icell
                  this%coef_p_pSigma(2, sg%vLevel), &    ! 3 stcl / num_icell
                  this%stcl_p_pX(4, sg%num_cell), &    ! 4 stcl / num_icell
                  this%stcl_p_pY(4, sg%num_cell), &
        ...
        DO i = 1, sg%num_icell
          ...
          this%stcl_p_pX(3, i) = sg%cell_stcl(8, i)
          this%stcl_p_pX(4, i) = sg%cell_stcl(9, i)

          this%coef_p_pX(1, i) = -1.0D0 / (2.0D0 * sg%cell_dist(6, i))
          this%coef_p_pX(2, i) = 1.0D0 / (2.0D0 * sg%cell_dist(6, i))
          this%coef_p_pX(3, i) = -1.0D0 / (2.0D0 * sg%cell_dist(6, i))
          this%coef_p_pX(4, i) = 1.0D0 / (2.0D0 * sg%cell_dist(6, i))

          this%stcl_p_pY(1, i) = sg%cell_stcl(5, i)
          this%stcl_p_pY(2, i) = sg%cell_stcl(6, i)
          this%stcl_p_pY(3, i) = sg%cell_stcl(8, i)
          this%stcl_p_pY(4, i) = sg%cell_stcl(9, i)

          this%coef_p_pY(1, i) = -1.0D0 / (2.0D0 * sg%cell_dist(8, i))
          this%coef_p_pY(2, i) = -1.0D0 / (2.0D0 * sg%cell_dist(8, i))
          this%coef_p_pY(3, i) = 1.0D0 / (2.0D0 * sg%cell_dist(8, i))
          this%coef_p_pY(4, i) = 1.0D0 / (2.0D0 * sg%cell_dist(8, i))
  ...
  IMPURE ELEMENTAL SUBROUTINE destructor(this)
    IMPLICIT NONE
    TYPE(UV2Divg_t), INTENT(INOUT) :: this
      ...
      BLOCK
        ...
        DO j = 1, X%sg%num_icell
          DO i = 1, X%sg%vLevel - 1
            ...
            IF ((X%sg%cell_stcl(6, j) == 0) .OR. (X%sg%cell_stcl(8, j) == 0)) CYCLE
            !First term
            DO hStcl = 1, 4
              divg(i, j, :) = divg(i, j, :) + &
            ...
            DO vStcl = 1, 2
              divg(i, j, :) = divg(i, j, :) &
                              - 0.25D0 * this%c1(i, j) * uwnd(this%stcl_p_pSigma(vStcl, i), X%sg%cell_stcl(5, j), :) * this%coef_p_pSigma(vStcl, i) &
                              - 0.25D0 * this%c1(i, j) * uwnd(this%stcl_p_pSigma(vStcl, i), X%sg%cell_stcl(6, j), :) * this%coef_p_pSigma(vStcl, i) &
                              - 0.25D0 * this%c1(i, j) * uwnd(this%stcl_p_pSigma(vStcl, i), X%sg%cell_stcl(8, j), :) * this%coef_p_pSigma(vStcl, i) &
                              - 0.25D0 * this%c1(i, j) * uwnd(this%stcl_p_pSigma(vStcl, i), X%sg%cell_stcl(9, j), :) * this%coef_p_pSigma(vStcl, i) &
                              - 0.25D0 * this%c2(i, j) * vwnd(this%stcl_p_pSigma(vStcl, i), X%sg%cell_stcl(5, j), :) * this%coef_p_pSigma(vStcl, i) &
                              - 0.25D0 * this%c2(i, j) * vwnd(this%stcl_p_pSigma(vStcl, i), X%sg%cell_stcl(6, j), :) * this%coef_p_pSigma(vStcl, i) &
                              - 0.25D0 * this%c2(i, j) * vwnd(this%stcl_p_pSigma(vStcl, i), X%sg%cell_stcl(8, j), :) * this%coef_p_pSigma(vStcl, i) &
                              - 0.25D0 * this%c2(i, j) * vwnd(this%stcl_p_pSigma(vStcl, i), X%sg%cell_stcl(9, j), :) * this%coef_p_pSigma(vStcl, i)

            END DO

            IF(i < X%sg%vLevel / 6) THEN
              divg(i, j, :) = divg(i, j, :) * EXP(i*1.0 - X%sg%vLevel / 6.0)
            END IF
            ! PRINT*, divg(i, j, :)
          END DO
        END DO
      ...
      BLOCK
        DO j = 1, dXi%sg%num_icell
          DO i = 1, dXi%sg%vLevel - 1
            IF ((dX%sg%cell_stcl(6, j) == 0) .OR. (dX%sg%cell_stcl(8, j) == 0)) CYCLE

            IF(i < dX%sg%vLevel / 6) THEN
              divg(i, j, :) = divg(i, j, :) * EXP(i*1.0 - dX%sg%vLevel / 6.0)
            END IF
            ...
            ! ! Second term
            DO vStcl = 1, 2
              uwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(5, j), :) = uwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(5, j), :) &
                                                                             - this%c1(i, j) * divg(i, j, :) * this%coef_p_pSigma(vStcl, i) * 0.25D0
              uwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(6, j), :) = uwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(6, j), :) &
                                                                             - this%c1(i, j) * divg(i, j, :) * this%coef_p_pSigma(vStcl, i) * 0.25D0
              uwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(8, j), :) = uwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(8, j), :) &
                                                                             - this%c1(i, j) * divg(i, j, :) * this%coef_p_pSigma(vStcl, i) * 0.25D0
              uwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(9, j), :) = uwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(9, j), :) &
                                                                             - this%c1(i, j) * divg(i, j, :) * this%coef_p_pSigma(vStcl, i) * 0.25D0
              vwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(5, j), :) = vwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(5, j), :) &
                                                                             - this%c2(i, j) * divg(i, j, :) * this%coef_p_pSigma(vStcl, i) * 0.25D0
              vwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(6, j), :) = vwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(6, j), :) &
                                                                             - this%c2(i, j) * divg(i, j, :) * this%coef_p_pSigma(vStcl, i) * 0.25D0
              vwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(8, j), :) = vwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(8, j), :) &
                                                                             - this%c2(i, j) * divg(i, j, :) * this%coef_p_pSigma(vStcl, i) * 0.25D0
              vwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(9, j), :) = vwnd(this%stcl_p_pSigma(vStcl, i), dX%sg%cell_stcl(9, j), :) &
                                                                             - this%c2(i, j) * divg(i, j, :) * this%coef_p_pSigma(vStcl, i) * 0.25D0
            END DO
