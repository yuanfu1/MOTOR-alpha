...
! C1 : InCompressiable weak constrain
  ...
  CONTAINS
    ...
    PROCEDURE, PUBLIC :: D_opr
  END TYPE JFunc_HybInc_t

CONTAINS

  SUBROUTINE initialize(this, configFile, X, Y, H, B, R, sg, Xb, B_e, Xb_enperts)
    IMPLICIT NONE
    CLASS(JFunc_HybInc_t) :: this
    ...
    TYPE(State_t), OPTIONAL :: Xb_enperts(:)
    ...
    IF (this%HybInc .AND. PRESENT(Xb_enperts)) THEN
      IF (ALLOCATED(this%Xb_enperts)) DEALLOCATE (this%Xb_enperts)
      ALLOCATE (this%Xb_enperts(this%ensNum))
      this%Xb_enperts = Xb_enperts
    END IF
    IF (this%HybInc) THEN
      this%Xb_inc = X
    END IF
    ...
    IF (this%Use_JcTerm_InCompres) THEN
      ...
      this%C1Coef = this%C1Coef * 1.0**sg%gLevel * 1.0D8
    ...
    IF (PRESENT(B_e)) THEN
      IF (this%Hybrid .OR. this%HybInc) THEN
        this%B_e => B_e
      END IF
    END IF
  ...
  SUBROUTINE get_J0_value(this, d0, X, J0, d0_e)
    CLASS(JFunc_HybInc_t) :: this
    REAL(r_kind), INTENT(INOUT) :: J0
    TYPE(State_t), INTENT(IN) :: d0
    TYPE(State_t), INTENT(IN), OPTIONAL :: d0_e(:)
    ...
    IF (this%HybInc) THEN
      IF (ALLOCATED(XX_einc)) DEALLOCATE (XX_einc)
      ALLOCATE (XX_einc(this%ensNum))

      IF (ALLOCATED(D_e)) DEALLOCATE (D_e)
      ALLOCATE (D_e(this%ensNum))
    END IF

    ! Update function value
    XX = this%B.SQRTINVMUL.d0

    ! Jiongming Pang added: 1) EnLoc as weak constraint like Hybrid; 2) HybInc (Hybrid in incremental analysis) referred to GSI
    IF (this%HybInc) THEN

      DO i = 1, this%ensNum
        XX_einc(i) = this%B_e.SQRTINVMUL. (d0_e(i))
      END DO

    ELSE IF (this%Hybrid) THEN

      d0e = d0
      XX_e = this%B_e.SQRTINVMUL.d0e
    ...
    IF (this%HybInc) THEN
      DO i = 1, this%ensNum
        CALL this%D_opr(XX_einc(i), this%Xb_enperts(i), Dsg_tmp)
        CALL this%H%Ctl2State%transBackward(Dsg_tmp)
        D_e(i) = this%R.SQRTINVMUL. (this%H%fwdTL_opr(Dsg_tmp, X))
      END DO
    ...
    J0 = (XX.DOT.XX)
    Jo = (D.DOT.D)
    IF (this%HybInc .OR. this%Hybrid) THEN
      J0 = J0 * this%beta_s
    END IF
    ...
    IF (this%HybInc) THEN
      DO i = 1, this%ensNum
        IXXeinc_tmp = this%B_e%sqrt_inverse_multiply_adjoint(XX_einc(i), X)
        J0 = J0 + (IXXeinc_tmp.DOT.d0_e(i)) * this%beta_e
      END DO
    ELSE IF (this%Hybrid) THEN
      J0 = J0 + (XX_e.DOT.XX_e) * this%beta_e
    END IF

    IF (this%HybInc) THEN
      DO i = 1, this%ensNum
        Jo = Jo + (D_e(i) .DOT.D_e(i))
      END DO
    END IF
  ...
  SUBROUTINE get_J_value(this, X, J, dXs, dXe)
    CLASS(JFunc_HybInc_t) :: this
    TYPE(State_t), INTENT(IN) :: X
    REAL(r_kind), INTENT(INOUT) :: J
    TYPE(State_t) :: XX, YY, CC, XX1, XX_e, Xe
    TYPE(ObsSet_t) :: DD
    TYPE(State_t), INTENT(IN), OPTIONAL :: dXs
    TYPE(State_t), INTENT(IN), OPTIONAL :: dXe(:)

    TYPE(State_t), ALLOCATABLE :: XX_einc(:), sigmaD_einc(:)
    TYPE(State_t) :: DAw_tmp, dX_total, IXXeinc_tmp
    INTEGER(r_kind) :: i, t
    ...
    IF (this%HybInc) THEN
      ...
      dX_total = dXs
      DO i = 1, this%ensNum
        XX_einc(i) = this%B_e.SQRTINVMUL. (dXe(i))
        CALL this%D_opr(XX_einc(i), this%Xb_enperts(i), DAw_tmp)
        CALL this%H%Ctl2State%transBackward(DAw_tmp)
        dX_total = dX_total + DAw_tmp
      END DO
      XX = this%B.SQRTINVMUL. (dXs)
      DD = this%R.SQRTINVMUL. ((this%H%fwdNL_opr(dX_total + this%Xb_inc)) - this%Y)
    ...
    IF (this%HybInc) THEN
      ...
      J = (XX.DOT.XX) * this%beta_s + (DD.DOT.DD)
      DO i = 1, this%ensNum
        IXXeinc_tmp = this%B_e%sqrt_inverse_multiply_adjoint(XX_einc(i), dX_total + this%Xb_inc)
        J = J + (IXXeinc_tmp.DOT.dXe(i)) * this%beta_e
      END DO
  ...
  SUBROUTINE get_J_and_grad_J_vec(this, X, J, grad_J, dXs, dXe, grad_Je)
    CLASS(JFunc_HybInc_t) :: this
    TYPE(State_t), INTENT(IN)   :: X
    REAL(r_kind), INTENT(INOUT)    :: J
    TYPE(State_t), INTENT(INOUT)   :: grad_J
    TYPE(State_t), INTENT(IN), OPTIONAL   :: dXs
    TYPE(State_t), INTENT(IN), OPTIONAL   :: dXe(:)
    TYPE(State_t), INTENT(INOUT), OPTIONAL   :: grad_Je(:)

    TYPE(State_t)     :: XX, XX1, grad_Jc, grad_Jb, grad_Jo, XTemp, XX_e, X_e
    ! TYPE(State_t) :: XmXb
    TYPE(ObsSet_t)    :: DD, D, YY
    REAL(r_kind)      :: Jb, Jo, Jc, Jb_s, Jb_e
    TYPE(State_t)     :: GG, HH, CC, GG_s, GG_e
    INTEGER(i_kind)   :: k, i, t

    TYPE(State_t), ALLOCATABLE :: XX_einc(:)
    TYPE(State_t) :: DAw_tmp, dX_total, grad_Jeinc_tmp, grad_Jeinc_otmp, IXXeinc_tmp, AHH
    ...
    IF (this%HybInc) THEN
      ...
      DO i = 1, this%ensNum
        IXXeinc_tmp = this%B_e%sqrt_inverse_multiply_adjoint(XX_einc(i), dX_total + this%Xb_inc)
        Jb = Jb + (IXXeinc_tmp.DOT.dXe(i)) * this%beta_e
      END DO
...
#ifdef TRACE_PRESSURE
    PRINT *, 'Jo is computed...'
#endif

    ! return
    J = Jb + Jo
!     IF (this%sg%isBaseProc()) &
!       WRITE(*,1) Jb, Jo, X%sg%gLevel,DD%getObsIdx('pres'), &
!         minval(DD%ObsFields(DD%getObsIdx('pres'))%values), &
!         maxval(DD%ObsFields(DD%getObsIdx('pres'))%values)
! 1     FORMAT('J-G: Jb',D16.6,' Jo',D16.6,' glevel: ',I2,' pres ObsIdx:',I2,' obsmm:',2D14.6)
    ...
    IF (this%HybInc) THEN
      ...
      GG_s = this%B%sqrt_inverse_multiply_adjoint(XX, dX_total + this%Xb_inc)
      GG = GG_s * this%beta_s
    ...
    IF (this%HybInc) THEN
      HH = this%H%adjMul_opr(YY, this%Xb_inc + dX_total) ! * this%beta_s
    ELSE
    ...
    IF (this%HybInc) THEN
      AHH = this%B_e.SQRTINVMUL.HH
      DO i = 1, this%ensNum
        CALL this%D_opr(AHH, this%Xb_enperts(i), grad_Jeinc_tmp)
        CALL this%H%Ctl2State%transBackward(grad_Jeinc_tmp)
        IXXeinc_tmp = this%B_e%sqrt_inverse_multiply_adjoint(XX_einc(i), dX_total + this%Xb_inc)
        grad_Je(i) = IXXeinc_tmp * 2.0D0 * this%beta_e + grad_Jeinc_tmp * 2.0D0
      END DO
    END IF
    ! grad_J = (this%B.SQRTINVMULADJ.XX)*2.0D0 + (this%H%adjMul_opr(this%R.SQRTINVMULADJ.DD, X))*2.0D0

    ! PRINT *,'grad_J pres min-max:', &
    !   MINVAL(grad_J%fields(grad_J%getVarIdx('pres_log'))%data), &
    !   MaxVAL(grad_J%fields(grad_J%getVarIdx('pres_log'))%data), ' G',X%sg%gLevel
...
1         FORMAT('Normalized Jos G', I1, ' itr:', I4, ' obsType ', A, 20(1X, A, E14.6))
  ...
  SUBROUTINE D_opr(this, XX, Xb_enperts, DAw)
    CLASS(JFunc_HybInc_t) :: this
    TYPE(State_t), INTENT(IN) :: XX, Xb_enperts
    TYPE(State_t), INTENT(INOUT) :: DAw
    INTEGER(i_kind) :: i, j, idv, idh, idt, nv, nh, nt

    DAw = XX%zeroCopy()
    DO i = LBOUND(XX%Fields, 1), UBOUND(XX%Fields, 1)
      DO j = LBOUND(Xb_enperts%Fields, 1), UBOUND(Xb_enperts%Fields, 1)
        IF (TRIM(XX%Fields(i)%Get_Name()) .EQ. TRIM(Xb_enperts%Fields(j)%Get_Name())) THEN
          DAw%Fields(i)%DATA = XX%Fields(i)%DATA * Xb_enperts%Fields(j)%DATA
        END IF
      END DO
    END DO

  END SUBROUTINE D_opr

  IMPURE ELEMENTAL SUBROUTINE destructor(this)
    IMPLICIT NONE
    TYPE(JFunc_HybInc_t), INTENT(INOUT) :: this

    PRINT *, 'End of destructor of JFunc_HybInc.'
  END SUBROUTINE destructor

END MODULE JFunc_HybInc_m
