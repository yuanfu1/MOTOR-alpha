...
! VERSION           : V 0.0
! HISTORY           : 2022-07-15, created by Jiongming Pang for LBUOY Obsercations.
!
!   Created by Jiongming Pang for LBUOY obs (pang.j.m@hotmail.com), 2021/07/15, @GBA-MWF, Shenzhen
...
!> @brief
!! This module implements a LBUOY data structure.
MODULE ObsLBUOY_m
  ...
  TYPE, EXTENDS(ObsBase_t) :: ObsLBUOY_t

  CONTAINS
    PROCEDURE :: ObsInitial => LBUOYInitial
    PROCEDURE :: ObsIngest => LBUOYIngest
    PROCEDURE :: ObsForward => LBUOYForward
    PROCEDURE :: ObsTangent => LBUOYTangent
    PROCEDURE :: ObsAdjoint => LBUOYAdjoint
    PROCEDURE :: ObsQC => LBUOYQC
  END TYPE ObsLBUOY_t

CONTAINS

  SUBROUTINE LBUOYInitial(this, configFile, idxFile)
    IMPLICIT NONE
    CLASS(ObsLBUOY_t) :: this
    ...
    ! read the obs LBUOY files names
    istatus = yaml_get_var(TRIM(configFile), 'IO', 'input_dir_LBUOY', obsFileDir)
    istatus = yaml_get_var(TRIM(configFile), 'IO', 'obsFileList_LBUOY', obsFileName)
    this%numfiles = UBOUND(obsFileName, 1)
    PRINT *, "amount of LBUOY obs files: ", this%numfiles

    ALLOCATE (this%fileNames(this%numfiles))
    DO i = 1, this%numfiles
      this%fileNames(i) = TRIM(obsFileDir)//"/"//TRIM(obsFileName(i))
      PRINT *, "name of LBUOY obs file ", i, ": ", TRIM(this%fileNames(i))
    END DO
  ...
  END SUBROUTINE LBUOYInitial

  SUBROUTINE LBUOYIngest(this, X)
    IMPLICIT NONE
    CLASS(ObsLBUOY_t) :: this
    TYPE(State_t) :: X
...
4       FORMAT('ObsLBUOY: LBUOY data file: ', A, ' is found and reading...')
      ELSE
        WRITE (*, 3) TRIM(this%fileNames(ifile))
3       FORMAT('ObsLBUOY: This LBUOY data file does not exist but the run continues, CHECK! ', A)
        CYCLE
      END IF
...
1     FORMAT('Number of LBUOY Obs in file ', I2 ' is: ', I4)
    END DO
    num_obs_total = SUM(num_sitein)
    WRITE (*, 2) num_obs_total
2   FORMAT('Total amount of LBUOY Obs in all files: ', I6)
    ...
    DO ifile = 1, num_dataFile
      ...
      DO isite = 1, sitenum
        ...
        IF (ABS(relv - missingValue) .GT. 0.01D0 .AND. relv .LT. 20.0D0) THEN
          obsHght(iobs) = relv
        ELSE
          obsHght(iobs) = 0.0D0
        END IF
  ...
  END SUBROUTINE LBUOYIngest

  FUNCTION LBUOYForward(this, X, O) RESULT(Y)
    IMPLICIT NONE
    CLASS(ObsLBUOY_t) :: this
    TYPE(State_t) :: X
    ...
    Y = O
    PRINT *, 'LBUOY: This forward operator has not implemented from ctl2State code, the code stops here'
    STOP
  END FUNCTION LBUOYForward

  FUNCTION LBUOYTangent(this, dX, X) RESULT(Y)
    IMPLICIT NONE
    CLASS(ObsLBUOY_t) :: this
    TYPE(State_t) :: dx, X
    TYPE(ObsSet_t) :: Y

    PRINT *, 'LBUOY: This tangent operator has not implemented from ctl2State code, the code stops here'
    STOP
  END FUNCTION LBUOYTangent

  FUNCTION LBUOYAdjoint(this, dY, X) RESULT(dX)
    IMPLICIT NONE
    CLASS(ObsLBUOY_t) :: this
    TYPE(State_t) :: dX, X
    TYPE(ObsSet_t) :: dY

    PRINT *, 'LBUOY: This adjoint operator has not implemented from ctl2State code, the code stops here'
    STOP
  END FUNCTION LBUOYAdjoint

  SUBROUTINE LBUOYQC(this)
    CLASS(ObsLBUOY_t) :: this
  END SUBROUTINE LBUOYQC

END MODULE ObsLBUOY_m
