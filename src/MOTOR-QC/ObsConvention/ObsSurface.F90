...
MODULE ObsSurface_m
  ...
  USE WriteVar_m
  USE conversions_m
  USE parameters_m, ONLY: degree2radian, missing, invalid
  ...
  CONTAINS
    ...
    PROCEDURE :: ObsForward => sfcForward
    PROCEDURE :: ObsTangent => sfcTangent
    PROCEDURE :: ObsAdjoint => sfcAdjoint
    PROCEDURE :: ObsQC => sfcQC
  END TYPE ObsSurface_t

  ! 假设这里 obsTime 是整数；stNames 长度5
  TYPE :: ObsItem
    INTEGER :: time
    CHARACTER(LEN=5) :: name
    INTEGER :: oldIndex     ! 保存原本在数组中的下标
  END TYPE ObsItem

CONTAINS

  ! 这里可以写一个比较子程序 / 或者写个接口
  ! 用于对 ObsItem 数组进行排序时的比较依据
  FUNCTION compareObs(a, b) RESULT(isLess)
    IMPLICIT NONE
    TYPE(ObsItem), INTENT(IN) :: a, b
    LOGICAL :: isLess
  ...
  END FUNCTION compareObs

  RECURSIVE SUBROUTINE quickSortObs(arr, left, right)
    IMPLICIT NONE
    TYPE(ObsItem), INTENT(INOUT) :: arr(:)
    INTEGER, INTENT(IN) :: left, right

    INTEGER :: i, j
    TYPE(ObsItem) :: pivot, temp
    ...
    DO
      DO WHILE (compareObs(arr(i), pivot))
        i = i + 1
      END DO
      DO WHILE (compareObs(pivot, arr(j)))
        j = j - 1
      END DO
      IF (i <= j) THEN
        ! swap
    ...
    IF (left < j) CALL quickSortObs(arr, left, j)
    IF (i < right) CALL quickSortObs(arr, i, right)
  END SUBROUTINE quickSortObs
  ...
  SUBROUTINE sfcInitial(this, configFile, idxFile)
    ...
    INTEGER :: i, istatus, vLevel

    ! SoloSite variables:
    CHARACTER(LEN=1024) :: soloFile
    LOGICAL :: soloSite

    ! Initialize obsCommon:
...
22      FORMAT('Bilinear interpolation scheme is used in the analysis')
    ...
    ELSE
      PRINT *, 'Reading Verify no SoloFile found'
    END IF
...
!     END DO
    ...
    DO i = 1, this%numVars
      ...
      CASE ('wspd', 'uwnd')
        ...
        IF (vLevel > 1) this%obsVars(i) = "windSpeed10min"
      CASE ('cdir', 'sdir', 'vwnd')
        this%obsVars(i) = "windDir"
        IF (vLevel > 1) this%obsVars(i) = "windDir10min"

      CASE ('uwnd10min')
        this%obsVars(i) = "windSpeed10min"
      CASE ('vwnd10min')
        this%obsVars(i) = "windDir10min"

      CASE ('uwnd2min')
        this%obsVars(i) = "windSpeed2min"
      CASE ('vwnd2min')
        this%obsVars(i) = "windDir2min"

      CASE ('pcpa5min')
        this%obsVars(i) = "precip5min"
...
12      FORMAT('SfcIngest - Obs variable is not defined: ', A, ' Check and rerun!')
    ...
    this%qcThreshold = 200000.0D0 ! Temporary holders
  ...
  SUBROUTINE sfcIngest(this, X)
    ...
    INTEGER(i_kind) :: uIdx, vIdx, cIdx, sIdx, wIdx, k
    INTEGER(i_kind) :: u10minIdx, v10minIdx, u2minIdx, v2minIdx
    REAL(r_kind) :: wspd, wdir, t1, t2
!#define soloCheck
      ...
      IF (nf_op .EQ. NF90_NOERR) THEN
        ...
        DO i = 1, this%numVars
          ...
          ELSE
            ...
            obsErrs(ind_obsin_beg:ind_obsin_end, i) = missing * (-1.0D0)   !-9999.0D0
          END IF
          ! IF (TRIM(this%obsVars(i)) .EQ. "stationPressure") &
          !   obsData(ind_obsin_beg:ind_obsin_end, i) = obsData(ind_obsin_beg:ind_obsin_end, i) !/100.0D0
        ...
        DO i = 1, this%numVars
          ...
          IF (TRIM(this%varNames(i)) .EQ. 'uwnd10min') u10minIdx = i
          IF (TRIM(this%varNames(i)) .EQ. 'vwnd10min') v10minIdx = i
          IF (TRIM(this%varNames(i)) .EQ. 'uwnd2min') u2minIdx = i
          IF (TRIM(this%varNames(i)) .EQ. 'vwnd2min') v2minIdx = i
        END DO
...
15        FORMAT('SfcIngest - User did not provided both uwnd and vwnd in obsVarList_Surface', 2I2, /, &
        ...
        ! For UV 10min options:
        IF (u10minIdx .NE. 0 .AND. v10minIdx .NE. 0) THEN
          ! Convert wind to uv:
          DO iuv = ind_obsin_beg, ind_obsin_end
            IF (ABS(obsData(iuv, u10minIdx)) .GT. ABS(missing) .OR. &
                ABS(obsData(iuv, v10minIdx)) .GT. ABS(missing) .OR. &
                obsData(iuv, v10minIdx) .LT. 0.0D0 .OR. &
                obsData(iuv, v10minIdx) .GT. 500.0D0) THEN
              obsData(iuv, u10minIdx) = invalid
              obsData(iuv, v10minIdx) = invalid
            ELSE
              wspd = obsData(iuv, u10minIdx)  ! Note that when uwnd is selected, the NC reads wind speed in uwnd position
              wdir = obsData(iuv, v10minIdx)  ! Note that when vwnd is selected, the NC reads wind direc in vwnd position
              CALL wind_to_uv(wspd, wdir, obsData(iuv, u10minIdx), obsData(iuv, v10minIdx))
              wspd = obsErrs(iuv, u10minIdx)
              wdir = obsErrs(iuv, v10minIdx)
              CALL wind_to_uv(wspd, wdir, obsErrs(iuv, u10minIdx), obsErrs(iuv, v10minIdx))
            END IF
          END DO
        END IF

        ! For UV 2 min options:
        IF (u2minIdx .NE. 0 .AND. v2minIdx .NE. 0) THEN
          ! Convert wind to uv:
          DO iuv = ind_obsin_beg, ind_obsin_end
            IF (ABS(obsData(iuv, u2minIdx)) .GT. ABS(missing) .OR. &
                ABS(obsData(iuv, v2minIdx)) .GT. ABS(missing) .OR. &
                obsData(iuv, v2minIdx) .LT. 0.0D0 .OR. &
                obsData(iuv, v2minIdx) .GT. 500.0D0) THEN
              obsData(iuv, u2minIdx) = invalid
              obsData(iuv, v2minIdx) = invalid
            ELSE
              wspd = obsData(iuv, u2minIdx)  ! Note that when uwnd is selected, the NC reads wind speed in uwnd position
              wdir = obsData(iuv, v2minIdx)  ! Note that when vwnd is selected, the NC reads wind direc in vwnd position
              CALL wind_to_uv(wspd, wdir, obsData(iuv, u2minIdx), obsData(iuv, v2minIdx))
              wspd = obsErrs(iuv, u2minIdx)
              wdir = obsErrs(iuv, v2minIdx)
              CALL wind_to_uv(wspd, wdir, obsErrs(iuv, u2minIdx), obsErrs(iuv, v2minIdx))
            END IF
          END DO
        END IF
        ...
        IF (uIdx .NE. 0 .AND. vIdx .NE. 0) THEN
          ! Convert wind to uv:
          DO iuv = ind_obsin_beg, ind_obsin_end
            IF (ABS(obsData(iuv, uIdx)) .GT. ABS(missing) .OR. &
                ABS(obsData(iuv, vIdx)) .GT. ABS(missing) .OR. &
                obsData(iuv, vIdx) .LT. 0.0D0 .OR. &
                obsData(iuv, vIdx) .GT. 500.0D0) THEN
              obsData(iuv, uIdx) = invalid
              obsData(iuv, vIdx) = invalid
            ELSE
        ...
        ELSE
          ...
          IF (cIdx .NE. 0 .AND. sIdx .NE. 0 .AND. wIdx .NE. 0) THEN
            ...
            ! Convert wind to uv:
            DO iuv = ind_obsin_beg, ind_obsin_end
              IF (ABS(obsData(iuv, cIdx)) .GT. ABS(missing) .OR. &
                  ABS(obsData(iuv, wIdx)) .GT. ABS(missing) .OR. &
                  obsData(iuv, cIdx) .LT. 0.0D0 .OR. &
                  obsData(iuv, wIdx) .GT. 500.0D0) THEN
                obsData(iuv, cIdx) = invalid
                obsData(iuv, sIdx) = invalid
                obsData(iuv, wIdx) = invalid
              ELSE
...
17          FORMAT('SfcIngest - incomplete wind vars: u: ', I2, ' v: ', I2, ' cdir: ', I2, ' sdir: ', I2, ' wspd: ', I2, /, &
    ...
    ind_obsin_end = 0
    DO ifile = 1, num_dataFile
      PRINT *, "obs file", ifile, TRIM(this%fileNames(ifile))
      ind_obsin_beg = ind_obsin_end + 1
      ind_obsin_end = ind_obsin_beg + ind_obsin(ifile) - 1
      PRINT *, "ind_obsin_beg and ind_obsin_end:", ind_obsin_beg, ind_obsin_end
      ALLOCATE (varstr)
    ...
    index_unique_tmp = (/(i, i=1, num_obs_total)/)
    ...
    ! Remove duplicate obs by qSort, by HYJ & QZL. 2025-01-10
    BLOCK
      TYPE(ObsItem), ALLOCATABLE :: obsArray(:)
      INTEGER(i_kind) :: num_obs_valid
      INTEGER, ALLOCATABLE :: index_unique_valid(:)

      num_obs_valid = COUNT(index_unique_judge .EQV. .TRUE.)
      index_unique_valid = PACK(index_unique_tmp, index_unique_judge)
      ALLOCATE (obsArray(num_obs_valid))

      DO i = 1, num_obs_valid
        obsArray(i)%time = obsTime(index_unique_valid(i))
        obsArray(i)%name = stNames(index_unique_valid(i)) (1:5)   ! 假设只取前5字符
        obsArray(i)%oldIndex = index_unique_valid(i)
      END DO

      CALL quickSortObs(obsArray, 1, num_obs_valid)

      index_unique_judge(obsArray(1)%oldIndex) = .TRUE.
      index_unique_tmp(obsArray(1)%oldIndex) = 1

      DO i = 2, num_obs_valid
        IF ((obsArray(i)%time == obsArray(i - 1)%time .AND. &
             obsArray(i)%name == obsArray(i - 1)%name)) THEN
          index_unique_judge(obsArray(i)%oldIndex) = .FALSE.
          index_unique_tmp(obsArray(i)%oldIndex) = 0
        ELSE
          index_unique_judge(obsArray(i)%oldIndex) = .TRUE.
          index_unique_tmp(obsArray(i)%oldIndex) = obsArray(i)%oldIndex
        END IF
        ! PRINT*, i, obsArray(i)%time, obsArray(i)%name, index_unique_judge(i)
      END DO

      DEALLOCATE (index_unique_valid, obsArray)
    END BLOCK

    WRITE (*, 333) t2 - t1, num_obs_total, X%sg%mpddInfo_sg%myrank
333 FORMAT('Time spent in removing duplicate SYNOP obs: ', E12.4, ' NumObs:', I8, ' at PC:', I2)

    ! Add soloSite blacklist to hold off some obs: Yuanfu Xie 2022-07-26
    ! IF (UBOUND(this%soloNames, 1) .GT. 0) THEN
    IF (ALLOCATED(this%soloNames)) THEN
      ...
      DO i = 1, num_obs_total
        ...
        DO ii = 1, UBOUND(this%soloNames, 1)
          IF (TRIM(stNames(i) (1:soloLen)) .EQ. TRIM(this%soloNames(ii))) THEN
            index_unique_judge(i) = .FALSE.
            index_unique_tmp(i) = 0
            WRITE (*, 13) i, ii, stNames(i) (1:5)
13          FORMAT('ObsSurface obs in Solo list station name: ', 2I6, 2X, A)
            CYCLE
          END IF
        END DO
#endif
...
12  FORMAT('SfcIngest - the amount of all input obs and accepted obs: ', 2I8, ' on proc: ', I3)
    ...
    DO i = 1, this%numObs
      DO j = 1, this%numVars
        IF (this%obsData(i, j) .GE. this%qcThreshold(j)) this%obsData(i, j) = missing
      END DO
    END DO
    ...
    BLOCK
      ...
      IF (X%getVarIdx('qvapor') /= 0) THEN
        ...
        IF (iq .NE. 0 .AND. ip .NE. 0 .AND. it .NE. 0) THEN
          DO j = 1, this%numObs
            IF (this%obsData(j, ip) == missing .OR. this%obsData(j, it) == missing &
                .OR. this%obsData(j, iq) == missing) THEN
              this%obsData(j, iq) = missing
            ELSE
              this%obsData(j, iq) = RH_to_qvapor(this%obsData(j, iq), this%obsData(j, it), this%obsData(j, ip))
            END IF
          END DO
        ELSE
          WRITE (*, 20) ip, iq, it
20        FORMAT('sfcIngest - As the analysis requests qvapor, sfc analysis variables must have pres, qvapor and temp present in your yaml', /, &
      ...
      IF (X%getVarIdx('rho') /= 0 .OR. X%getVarIdx('rho_ctl') /= 0) THEN
        ...
        it = this%getVarIdx('temp')
        DO j = 1, this%numObs
          IF (this%obsData(j, ip) == missing .OR. this%obsData(j, it) == missing) THEN
            this%obsData(j, iq) = missing
          ELSE
      ...
      ! Pressure in hPa:
      ! DO j = 1, this%numObs
      !   IF (this%obsData(j, ip) .NE. missing) this%obsData(i, ip) = this%obsData(i, ip)
      ! END DO

    END BLOCK

    DO i = 1, this%numVars
      WRITE (*, 31) i, this%numVars, TRIM(this%varNames(i)), missing, invalid, X%mpddGlob%myrank
31    FORMAT('SfcIngest valid obs:', I2, ' of total', I2, ' var ', A20, ' missing/invalid vals:', 2D14.6, ' pc', I2)
      WRITE (*, 32) i, TRIM(this%varNames(i)), &
        MAXVAL(this%obsData(:, i), &
               !MASK=(this%obsData(:, i) .ne. missing .and. this%obsData(:, i) .le. invalid)), &
               MASK=(this%obsData(:, i) .NE. missing)), &
        MINVAL(this%obsData(:, i), &
               MASK=(this%obsData(:, i) .NE. missing .AND. this%obsData(:, i) .LE. invalid)), &
        COUNT(ABS(this%obsData(:, i)) .LT. missing), TRIM(this%obsType), X%mpddGlob%myrank
32    FORMAT('SfcIngest obs range/counts: ', I2, ' ', A20, 2D14.6, ' count:', I6, ' obsType ', A, ' pc', I2)
    END DO
    ...
    ! Added by Zilong for QC
    DO i = 1, this%numVars
      SELECT CASE (TRIM(this%varNames(i)))
      CASE ('temp')
        this%tempIdx = i
        this%qcThreshold(this%tempIdx) = 10.0D0
        IF (X%sg%vLevel == 1) this%qcThreshold(i) = 15.0D0
      CASE ('qvapor')
        this%qvaporIdx = i
        this%qcThreshold(this%qvaporIdx) = 0.008D0
        IF (X%sg%vLevel == 1) this%qcThreshold(i) = 0.02D0
      CASE ('uwnd')
        this%qcThreshold(i) = 10.0D0
        IF (X%sg%vLevel == 1) this%qcThreshold(i) = 20.0D0
      CASE ('vwnd')
        this%qcThreshold(i) = 10.0D0
        IF (X%sg%vLevel == 1) this%qcThreshold(i) = 20.0D0
      CASE ('pres')
        this%presIdx = i
        this%qcThreshold(this%presIdx) = 1000.0D0
        IF (X%sg%vLevel == 1) this%qcThreshold(i) = 2000.0D0
      CASE ('psl')
        this%qcThreshold(i) = 1000.0D0
        IF (X%sg%vLevel == 1) this%qcThreshold(i) = 6000.0D0
      CASE ('lnp')
        this%lnpIdx = i
        this%qcThreshold(this%lnpIdx) = 1000.0D0
        IF (X%sg%vLevel == 1) this%qcThreshold(i) = 6000.0D0
      CASE ('pcpa')
        this%qcThreshold(i) = 300.0D0
      CASE ('uwnd10min')
    ...
    ! Convert the observed values to obsSpace in the assimilation:
    DO j = 1, UBOUND(this%obsCommon%varObsSpace, 1) !this%numObs
      WRITE (*, 22) j, TRIM(this%obsCommon%varObsSpace(j)), X%sg%mpddInfo_sg%myrank
22    FORMAT('varObsSpace: ', I2, A, ' pc:', I2)
      SELECT CASE (TRIM(this%obsCommon%varObsSpace(j)))
      CASE ('lnp')
        DO i = 1, this%numVars
          IF (TRIM(this%varNames(i)) .EQ. 'pres') THEN
            PRINT *, 'Pressure before obs2obsSpace...', this%obsData(1, i), MAXVAL(this%obsData(:, i))
            CALL this%obsCommon%obs2obsSpace(this%numObs, TRIM(this%varNames(i)), &
                                             TRIM(this%obsCommon%varObsSpace(j)), this%obsData(:, i))
            this%varNames(i) = 'lnp'
            PRINT *, 'Pressure  after obs2obsSpace...', this%obsData(1, i), MAXVAL(this%obsData(:, i))
          END IF
        END DO
      CASE ('psl')
        DO i = 1, this%numVars
          IF (TRIM(this%varNames(i)) .EQ. 'pres') THEN
            PRINT *, 'Pressure before psfc to psl...', this%obsData(1, i), MAXVAL(this%obsData(:, i))
            DO k = 1, this%numObs
              IF (this%obsData(k, this%tempIdx) == missing .OR. this%obsData(k, i) == missing .OR. this%obsHght(k) == missing) THEN
                this%obsData(k, i) = missing
              ELSE
                this%obsData(k, i) = psfc_to_psl(this%obsData(k, i), this%obsHght(k), this%obsData(k, this%tempIdx), X%sg%tmsl)
              END IF
            END DO

            this%varNames(i) = 'psl'
            PRINT *, 'Pressure  after psfc to psl...', this%obsData(1, i), MAXVAL(this%obsData(:, i))
          END IF
        END DO
      END SELECT
    ...
    END DO
    DO i = 1, this%numVars
      WRITE (*, 21) i, TRIM(this%varNames(i)), X%sg%mpddInfo_sg%myrank
21    FORMAT('Converting obs to obsSpace var:', I2, ' name:', A, ' pc:', I2)
    END DO
  ...
  FUNCTION sfcForward(this, X, O) RESULT(Y)
    IMPLICIT NONE
    ...
    Y = O%zeroCopy() ! Initiate Y with the sample obs

    ! This operator assumes that there is not data overlapping from this data type:
    DO i = 1, UBOUND(Y%ObsFields, 1)
      ...
      END DO
    END DO
  END FUNCTION sfcForward

  FUNCTION sfcTangent(this, dX, X) RESULT(Y)
    IMPLICIT NONE
    ...
    PRINT *, 'Synop: Testing tangent operator comparing to ctl2State code, the code stops here'
    ! DO i = 1, UBOUND(Y%ObsFields, 1)
    !   DO j = LBOUND(X%fields, 1), UBOUND(X%fields, 1)
    !     IF ((TRIM(Y%ObsFields(i)%Get_Name())) .EQ. (TRIM(X%fields(j)%Get_Name()))) THEN
    !       DO k = LBOUND(Y%ObsFields(i)%idx, 1), UBOUND(Y%ObsFields(i)%idx, 1)
    ...
    ! END DO
    STOP
  END FUNCTION sfcTangent

  FUNCTION sfcAdjoint(this, dY, X) RESULT(dX)
    IMPLICIT NONE
