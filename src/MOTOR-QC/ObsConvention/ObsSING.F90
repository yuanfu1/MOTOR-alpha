...
! VERSION           : V 0.0
! HISTORY           : 2022-07-15, created by Jiongming Pang for SING Obsercations.
!
!   Created by Jiongming Pang for SING obs (pang.j.m@hotmail.com), 2021/07/15, @GBA-MWF, Shenzhen
...
!> @brief
!! This module implements a SING data structure.
MODULE ObsSING_m
  ...
  TYPE, EXTENDS(ObsBase_t) :: ObsSING_t

  CONTAINS
    PROCEDURE :: ObsInitial => SINGInitial
    PROCEDURE :: ObsIngest => SINGIngest
    PROCEDURE :: ObsForward => SINGForward
    PROCEDURE :: ObsTangent => SINGTangent
    PROCEDURE :: ObsAdjoint => SINGAdjoint
    PROCEDURE :: ObsQC => SINGQC
  END TYPE ObsSING_t

CONTAINS

  SUBROUTINE SINGInitial(this, configFile, idxFile)
    IMPLICIT NONE
    CLASS(ObsSING_t) :: this
    ...
    ! read the obs SING files names
    istatus = yaml_get_var(TRIM(configFile), 'IO', 'input_dir_SING', obsFileDir)
    istatus = yaml_get_var(TRIM(configFile), 'IO', 'obsFileList_SING', obsFileName)
    this%numfiles = UBOUND(obsFileName, 1)
    PRINT *, "amount of SING obs files: ", this%numfiles

    ALLOCATE (this%fileNames(this%numfiles))
    DO i = 1, this%numfiles
      this%fileNames(i) = TRIM(obsFileDir)//"/"//TRIM(obsFileName(i))
      PRINT *, "name of SING obs file ", i, ": ", TRIM(this%fileNames(i))
    END DO
    ...
    this%varNames(2) = "uwnd"
    this%varNames(3) = "vwnd"
  ...
  END SUBROUTINE SINGInitial

  SUBROUTINE SINGIngest(this, X)
    IMPLICIT NONE
    CLASS(ObsSING_t) :: this
    TYPE(State_t) :: X
    ...
    REAL(r_kind) :: p, u, v, rh, q, vwu, hd, vd, pcpa01hr, pcpa06hr, pcpa24hr
...
4       FORMAT('ObsSING: SING data file: ', A, ' is found and reading...')
      ELSE
        WRITE (*, 3) TRIM(this%fileNames(ifile))
3       FORMAT('ObsSING: This SING data file does not exist but the run continues, CHECK! ', A)
        CYCLE
      END IF
...
1     FORMAT('Number of SING Obs in file ', I2 ' is: ', I4)
    END DO
    num_obs_total = SUM(num_sitein)
    WRITE (*, 2) num_obs_total
2   FORMAT('Total amount of SING Obs in all files: ', I6)
    ...
    DO ifile = 1, num_dataFile
      ...
      DO isite = 1, sitenum
        READ (101, '(a7, 1x, 4f10.2)') siteID, rlat, rlon, rday, rhhmm
        READ (101, '(4f10.2)') relv, t, winddir, windspd
        ...
        IF (ABS(windspd - missingValue) .LT. 0.01D0 .OR. &
          ...
          obsData(iobs, 2) = missing   ! uwnd
          obsData(iobs, 3) = missing   ! vwnd

        ELSE

          CALL wind_to_uv(windspd, winddir, obsData(iobs, 2), obsData(iobs, 3))

        END IF

        IF (ABS(relv - missingValue) .GT. 0.01D0) THEN
          obsHght(iobs) = relv
        ELSE
    ...
    PRINT *, "DONE SINGIngest"

    this%qcThreshold(1) = 8.0D0
    this%qcThreshold(2) = 8.0D0
    this%qcThreshold(3) = 8.0D0

  END SUBROUTINE SINGIngest

  FUNCTION SINGForward(this, X, O) RESULT(Y)
    IMPLICIT NONE
    CLASS(ObsSING_t) :: this
    TYPE(State_t) :: X
    ...
    Y = O
    PRINT *, 'This forward operator has not implemented from ctl2State code, the code stops here'
    STOP
  END FUNCTION SINGForward

  FUNCTION SINGTangent(this, dX, X) RESULT(Y)
    IMPLICIT NONE
    CLASS(ObsSING_t) :: this
    TYPE(State_t) :: dX, X
    TYPE(ObsSet_t) :: Y

    PRINT *, 'SING: This tangent operator has not implemented from ctl2State code, the code stops here'
    STOP
  END FUNCTION SINGTangent

  FUNCTION SINGAdjoint(this, dY, X) RESULT(dX)
    IMPLICIT NONE
    CLASS(ObsSING_t) :: this
    TYPE(State_t) :: dX, X
    TYPE(ObsSet_t) :: dY

    PRINT *, 'SING: This adjointoperator has not implemented from ctl2State code, the code stops here'
    STOP
  END FUNCTION SINGAdjoint

  SUBROUTINE SINGQC(this)
    CLASS(ObsSING_t) :: this
  END SUBROUTINE SINGQC

END MODULE ObsSING_m
