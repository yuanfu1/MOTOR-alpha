...
MODULE GrapesModelvarIO_500m_m
  ...
  END TYPE GrapesModelvarIO_500m_t

  INTERFACE GrapesModelvarIO_500m_t
    PROCEDURE :: constructor
  END INTERFACE

CONTAINS
  FUNCTION constructor(modvarFns, modvarCtlFn, mdate, vLevel) RESULT(this)
    TYPE(GrapesModelvarIO_500m_t) :: this
  ...
  SUBROUTINE get_domain(this)
    IMPLICIT NONE
    CLASS(GrapesModelvarIO_500m_t) :: this
    INTEGER                   :: fid
    INTEGER                   :: i
  ...
  SUBROUTINE read_value(this)
    ...
    ALLOCATE (this%cldt_s(idn, jdn, nt))
    ...
    IF (this%vLevel == 1) THEN
      DO t = 1, nt
        ...
        ! read cldt
        READ (fid, pos=iprec) this%cldt_s(:,:,t)
        this%cldt = this%cldt_s
        iprec = iprec + idn * jdn * 4
        PRINT *, 'min and max of cldt:', MINVAL(this%cldt), MAXVAL(this%cldt)
        PRINT *, 'done cldt reading ...'
  ...
  IMPURE ELEMENTAL SUBROUTINE destructor(this)
    TYPE(GrapesModelvarIO_500m_t), INTENT(INOUT) :: this
    ...
    IF (ALLOCATED(this%cldt_s)) DEALLOCATE(this%cldt_s)

    PRINT *, 'destructor works'

  END SUBROUTINE destructor

END MODULE GrapesModelvarIO_500m_m
