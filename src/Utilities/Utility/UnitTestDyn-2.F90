...
CONTAINS
  ...
  SUBROUTINE RH_psi_func(this, attime, latlon, zhght, stream, vortct)
    ...
    ! Local variables:
    INTEGER(i_kind) :: i, k
    REAL(r_kind)    :: a0, R0, clat, slat, tlat, clon, slon, clatR(0:5)
    ...
    DO k = 1, this%vLevel
      DO i = 1, this%num_cell
        ...
        ! Stream function:
        stream(k, i) = &
          -R0 * R0 * slat * (rh_omga - rh_kapa * clatR(3) * clon)

        ! Relative vorticity:
        vortct(k, i) = slat * (2.0D0 * rh_omga - rh_kapa * clatR(3) * &
    ...
    !!! use the above stream function and vorticity only
        ...
        this%psi1st(1, k, i) = 2.0D0 / R0 * stream(k, i)
        this%psi1st(2, k, i) = -R0 * R0 * (rh_omga * clat - rh_kapa * &
                                           ((rh_bigR + 1.0D0) * clatR(4) - rh_bigR * clatR(2)) * clon)
        this%psi1st(3, k, i) = -R0 * R0 * rh_kapa * rh_bigR * clatR(3) * slat * slon
        this%psi1st_dclat(1, k, i) = -R0 * R0 * (rh_omga - rh_kapa * &
                                                 ((rh_bigR + 1.0D0) * clatR(3) - rh_bigR * clatR(1)) * clon)
        this%psi1st_dclat(2, k, i) = -R0 * R0 * rh_kapa * rh_bigR * clatR(2) * slat * slon
        ...
        this%psi2nd(1, k, i) = R0 * R0 * slat * (rh_omga + rh_kapa * &
  ...
  SUBROUTINE RH_chi_func(this, attime, latlon, zhght, potential, divergence)
    ...
    DO k = 1, this%vLevel
      DO i = 1, this%num_cell
        ...
        ! potential function:
        potential(k, i) = &
          -R0 * R0 * slat * (rh_omga_t - rh_kapa_t * clatR(3) * clon)

        ! divergence:
        divergence(k, i) = slat * (2.0D0 * rh_omga_t - rh_kapa_t * clatR(3) * &
                                   (rh_bigR**2 + 3.0D0 * rh_bigR + 2.0D0) * clon)

        ! The first order derivatives: for the last dimension, chi: (1) R0, (2) lat, (3) lon; del: (1) lat, (2) lon
        ! *_dclat are parameters divived by cos(lat)
        this%chi1st(1, k, i) = 2.0D0 / R0 * potential(k, i)
        this%chi1st(2, k, i) = -R0 * R0 * (rh_omga_t * clat - rh_kapa_t * &
                                           ((rh_bigR + 1.0D0) * clatR(4) - rh_bigR * clatR(2)) * clon)
        this%chi1st(3, k, i) = -R0 * R0 * rh_kapa_t * rh_bigR * clatR(3) * slat * slon
        this%chi1st_dclat(1, k, i) = -R0 * R0 * (rh_omga_t - rh_kapa_t * &
                                                 ((rh_bigR + 1.0D0) * clatR(3) - rh_bigR * clatR(1)) * clon)
        this%chi1st_dclat(2, k, i) = -R0 * R0 * rh_kapa_t * rh_bigR * clatR(2) * slat * slon
        ...
        ! The second order derivatives: 1 lat_lat; 2 lat_lon; 3 lon_lon:
        this%chi2nd(1, k, i) = R0 * R0 * slat * (rh_omga_t + rh_kapa_t * &
                                                 ((rh_bigR * (rh_bigR - 1.0D0) * clatR(1) - &
                                                   (rh_bigR + 1.0D0)**2 * clatR(3))) * clon)
        ...
        this%chiThetaLambda_dclat(k, i) = -R0 * R0 * rh_kapa_t * rh_bigR * &
                                          ((rh_bigR + 1.0D0) * clatR(3) - rh_bigR * clatR(1)) * slon
        this%chi2ndLambda_dclat(k, i) = -R0 * R0 * rh_kapa_t * rh_bigR**2 * clatR(2) * slat * clon
    ...
    DO k = 1, this%vLevel
      ...
      DO i = 1, this%num_cell
        ...
        this%DM1st(1, k, i) = 1.0D0 / R0**2 * (this%Hz1sttheta(k, i) * this%z1sttheta(k, i) * this%chi1st(2, k, i) &
                              ...
                              + 1.0D0 / R0**2 * (this%z1sttheta_dclat(k, i) * this%psi2nd(2, k, i) &
                                                 + this%d_z1stthetadclat_dtheta(k, i) * this%psi1st(3, k, i))
    ...
    DO k = 1, this%vLevel
      ...
      DO i = 1, this%num_cell
        ...
        F_eta_chi(k, i) = ((this%zta1st(1, k, i) + 2.0D0 * Omega * clat) * this%chi1st(2, k, i) + &
                           this%zta1st_dclat(2, k, i) * this%chi1st_dclat(2, k, i)) / a0 / a0 + &
                          (vortct(k, i) + 2.0D0 * Omega * slat) * divergence(k, i)
        F_DM_psisigma(k, i) = 2.0D0 / a0**2 / R0 * this%DM1st(1, k, i) * this%psi1st(2, k, i) &
                              + 2.0D0 / a0**2 / R0 * this%DM1stLambda_dclat(k, i) * this%psi1st_dclat(2, k, i) &
                              + 2.0D0 / R0 * DM(k, i) * vortct(k, i)

        J_DM_chisigma(k, i) = 2.0D0 / a0**2 / R0 * this%DM1st(2, k, i) * this%chi1st_dclat(1, k, i) &
                              - 2.0D0 / a0**2 / R0 * this%DM1st(1, k, i) * this%chi1st_dclat(2, k, i)
