...
CONTAINS
  ...
  SUBROUTINE rho_func(this, attime, latlon, z, z_s, rho, par_rho_t)
    IMPLICIT NONE
    CLASS(UnitTestDyn_t) :: this
    REAL(r_kind), INTENT(IN) ::  latlon(:, :), z(:, :), z_s(:), attime
    ...
    DO k = 1, this%vLevel
      DO i = 1, this%num_cell
        ...
        zratio = (1.0D0 - this%sigma(k) / this%z_top)
        rho(k, i) = rho0 * (1.0D0 + omg * slat) * expz * ct
        this%rho1stsigma(k, i) = -alpha * rho(k, i) * (this%z_top - z_s(i)) / this%z_top
        this%rho1sttheta(k, i) = rho0 * omg * clat * expz * ct - alpha * rho(k, i) * zratio * this%zs1sttheta(i)

        IF (PRESENT(par_rho_t)) THEN
          par_rho_t(k, i) = -rho0 * (1.0D0 + omg * slat) * expz * st * rh_angl
        END IF
  ...
  SUBROUTINE Tp_func(this, attime, latlon, zhght, Tp, par_Tp_t)
    IMPLICIT NONE
    CLASS(UnitTestDyn_t) :: this
    REAL(r_kind), INTENT(IN) ::  latlon(:, :), zhght(:, :), attime
  ...
  SUBROUTINE q_func(this, attime, latlon, zhight, q, par_q_t)
    ...
    ! Local variables:
    INTEGER(i_kind) :: i, k
    REAL(r_kind)    :: omg, q0, alpha, clat, slat, ct, st, &
                       zfun, hfun
    ...
    DO k = 1, this%vLevel
      DO i = 1, this%num_cell
        ...
        q(k, i) = q0 * zfun * hfun * ct
        this%q1sttheta(k, i) = -q0 * zfun * omg * slat * ct
        this%q1stsigma(k, i) = -alpha * q(k, i)

        IF (PRESENT(par_q_t)) THEN
          par_q_t(k, i) = -q0 * zfun * hfun * st * rh_angl
        END IF
  ...
  SUBROUTINE TpTen(this, latlon, psi, chi, z, Hz, tenTp, X1)
    IMPLICIT NONE
    CLASS(UnitTestDyn_t) :: this
    TYPE(State_t), INTENT(INOUT), OPTIONAL :: X1
    REAL(r_kind), INTENT(IN) :: latlon(:, :), psi(:, :), chi(:, :), &
    ...
    REAL(r_kind), ALLOCATABLE, DIMENSION(:, :) :: J_Tp_psi, F_Tp_chi, J_z_psi, F_z_chi
    ...
    DEALLOCATE (J_Tp_psi, F_Tp_chi, J_z_psi, F_z_chi)

  END SUBROUTINE

  SUBROUTINE qTen(this, latlon, psi, chi, z, Hz, tenq, X1)
    IMPLICIT NONE
    CLASS(UnitTestDyn_t) :: this
    TYPE(State_t), INTENT(INOUT), OPTIONAL :: X1
    REAL(r_kind), INTENT(IN) :: latlon(:, :), psi(:, :), chi(:, :), &
    ...
    REAL(r_kind), ALLOCATABLE, DIMENSION(:, :) :: J_q_psi, F_q_chi, J_z_psi, F_z_chi
    ...
    DEALLOCATE (J_q_psi, F_q_chi, J_z_psi, F_z_chi)
  ...
  SUBROUTINE righthandsHalf(this, t0, XHalf)
    ...
    ASSOCIATE (sg => XHalf%sg, &
      ...
      CALL this%q_func(t0, sg%cell_cntr, this%sigma3d, fields(q_idxt)%DATA(:, :, 1))
      ...
      CALL this%RhoTen(sg%cell_cntr, fields(rho_idxt)%DATA(:, :, 1), fields(div_idxt)%DATA(:, :, 1), &
                       fields(psi_idxt)%DATA(:, :, 1), fields(chi_idxt)%DATA(:, :, 1), &
                       this%DM, this%zHght, this%Hz, fields(trho_idxt)%DATA(:, :, 1))
      CALL this%qTen(sg%cell_cntr, fields(psi_idxt)%DATA(:, :, 1), fields(chi_idxt)%DATA(:, :, 1), &
                     this%zHght, this%Hz, fields(tq_idxt)%DATA(:, :, 1))
  ...
  SUBROUTINE righthandsFull(this, t0, XFull)
    ...
    ASSOCIATE (sg => XFull%sg, &
      ...
      CALL this%TpTen(sg%cell_cntr, fields(psi_idxt)%DATA(:, :, 1), fields(chi_idxt)%DATA(:, :, 1), &
                      this%zHght, this%Hz, fields(ttheta_idxt)%DATA(:, :, 1))
    END ASSOCIATE
