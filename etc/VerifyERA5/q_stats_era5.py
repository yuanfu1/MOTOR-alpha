...
old_ds = xr.open_dataset(f'{root}/ctl-T10.9/MOTOR-3DVar_ana_{glvl}.nc')
...
ea5_gd_q_bg_intp = era5_ds.q.interp(latitude=lats, longitude=lons, valid_time=times[-1], pressure_level=press[-1,:,:,:]/100.0)

print(press)

era5_q = ea5_gd_q_bg_intp
old_q = old_ds.qvapor[-1,:,:,:]
new_q = new_ds.qvapor[-1,:,:,:]
bcg_q = bcg_ds.qvapor[-1,:,:,:]
