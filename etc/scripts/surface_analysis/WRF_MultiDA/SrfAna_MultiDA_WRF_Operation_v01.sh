...
#
  ...
  tmp=`echo ${tmp}|awk '{print int($1)==$1?$1:int(int($1*10/10+1))}'`
...
#
  ...
  SAVE_UNIXTIME=$(( ${FINL_UNIXTIME_ANA} - 48 * 60 * 60 ))
  TIME_SAVE=`date "+%Y%m%d_%H%M" -d @${SAVE_UNIXTIME}`
  ...
  mkdir -p ${anaDIR}
  mkdir -p ${bakDIR}
  mv ${OUTDATADIR}/*ana*${MON_SAVE}*_${DAY_SAVE}*.nc ${anaDIR}/
  mv ${OUTDATADIR}/*bak*${MON_SAVE}*_${DAY_SAVE}*.nc ${bakDIR}/
